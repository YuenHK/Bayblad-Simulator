...
async function fixture(mutable = false) {
  ...
  return {artifact,env,log,manifestSha:createHash("sha256").update(manifest).digest("hex"),processEnv:{...process.env,PATH:`${bin}:${process.env.PATH}`,DOCKER_LOG:log,DEPLOYMENT_AUTHORIZATION_PURPOSE:"release-integration"}};
}
describe("production deploy wrapper",()=>{
  it("rejects a mutable tag before invoking Compose",async()=>{const f=await fixture(true);await expect(execute("./scripts/deploy-production.sh",[f.artifact,f.env,f.manifestSha,"school/top","a".repeat(40)],{env:f.processEnv})).rejects.toBeTruthy();await expect(readFile(f.log,"utf8")).rejects.toMatchObject({code:"ENOENT"});});
  it("runs config, pull and up only after exact external authorization binding",async()=>{const f=await fixture();await execute("./scripts/deploy-production.sh",[f.artifact,f.env,f.manifestSha,"school/top","a".repeat(40)],{env:f.processEnv});const calls=await readFile(f.log,"utf8");expect(calls.indexOf(" config --quiet")).toBeLessThan(calls.indexOf(" pull"));expect(calls.indexOf(" pull")).toBeLessThan(calls.indexOf(" up -d --wait"));});
  it("rejects a manifest digest not supplied by the artifact itself",async()=>{const f=await fixture();await expect(execute("./scripts/deploy-production.sh",[f.artifact,f.env,"f".repeat(64),"school/top","a".repeat(40)],{env:f.processEnv})).rejects.toBeTruthy();});
});
