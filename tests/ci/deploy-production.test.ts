...
async function fixture(mutable = false) {
  ...
  const images={server:`ghcr.io/school/top/server@${digest("1")}`,migration:`ghcr.io/school/top/server@${digest("1")}`,web:`ghcr.io/school/top/web@${digest("2")}`,database:`ghcr.io/school/top/database@${digest("3")}`};
  ...
  return {artifact,env,log,processEnv:{...process.env,PATH:`${bin}:${process.env.PATH}`,DOCKER_LOG:log}};
}
describe("production deploy wrapper",()=>{
  it("rejects a mutable tag before invoking Compose",async()=>{const f=await fixture(true);await expect(execute("./scripts/deploy-production.sh",[f.artifact,f.env],{env:f.processEnv})).rejects.toBeTruthy();await expect(readFile(f.log,"utf8")).rejects.toMatchObject({code:"ENOENT"});});
  it("runs config, pull and up only after exact manifest binding",async()=>{const f=await fixture();await execute("./scripts/deploy-production.sh",[f.artifact,f.env],{env:f.processEnv});const calls=await readFile(f.log,"utf8");expect(calls.indexOf(" config --quiet")).toBeLessThan(calls.indexOf(" pull"));expect(calls.indexOf(" pull")).toBeLessThan(calls.indexOf(" up -d --wait"));});
});
