...
type Options = Readonly<{ wrongRoot?: boolean; wrongSubject?: boolean; wrongSource?: boolean; wrongLayerDigest?: boolean; sbomOnly?: boolean; duplicatePlatform?: boolean; dockerIndex?: boolean; swapBody?: boolean }>;
function platformEvidence(architecture: "amd64" | "arm64", options: Options) {
  ...
  const provenance = { _type: "https://in-toto.io/Statement/v0.1", subject: [{ name: `pkg:docker/ghcr.io/${repository}/steam-top/server?platform=linux%2F${architecture}`, digest: { sha256: subjectDigest } }], predicateType: "https://slsa.dev/provenance/v0.2", predicate: { builder: { id: "" }, buildType: "https://mobyproject.org/buildkit@v1", invocation: { environment: { platform: `linux/${architecture}` } }, metadata: { "https://mobyproject.org/buildkit@v1#metadata": { vcs: { source: `https://github.com/${repository}.git`, revision: sourceRevision } } } } };
  const sbom = { _type: "https://in-toto.io/Statement/v0.1", subject: [{ name: "_", digest: { sha256: image(architecture).digest.slice(7) } }], predicateType: "https://spdx.dev/Document", predicate: { SPDXID: "SPDXRef-DOCUMENT", spdxVersion: "SPDX-2.3", creationInfo: { creators: ["Tool: buildkit"] } } };
...
describe("multi-platform release image verifier", () => {
  ...
  it.each([
    ["root digest", { wrongRoot: true }], ["in-toto subject", { wrongSubject: true }], ["source revision", { wrongSource: true }], ["layer digest", { wrongLayerDigest: true }], ["missing SBOM", { sbomOnly: true }], ["duplicate platform", { duplicatePlatform: true }], ["platform body swap", { swapBody: true }],
