...
const fixture=()=>{
  ...
  writeEvidence();
  const run=(overrides:{intent?:any;anchor?:any;bundle?:string;expectedRun?:number;expectedAttempt?:number;expectedBundle?:string;evidenceMutate?:(value:any)=>void}={})=>{if(overrides.anchor){writeFileSync(anchorPath,`${JSON.stringify(overrides.anchor)}\n`)}writeFileSync(bundlePath,overrides.bundle??"valid-bundle");writeEvidence(overrides.intent??intent,overrides.evidenceMutate);return spawnSync("scripts/verify-and-create-production-policy-entry.sh",[intentPath,bundlePath,anchorPath,repository,String(overrides.expectedRun??runId),String(overrides.expectedAttempt??runAttempt),overrides.expectedBundle??bundleSha,"root-key",output],{cwd:process.cwd(),env:{...process.env,PATH:`${fakeBin}:${process.env.PATH}`,FAKE_GH_EVIDENCE:evidencePath},encoding:"utf8"})};
