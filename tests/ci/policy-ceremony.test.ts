...
const fixture=()=>{
  ...
  mkdirSync(fakeBin);
  writeFileSync(gh,"#!/bin/bash\nset -eu\nexpected=(attestation verify \"$FAKE_GH_INTENT\" --repo \"$FAKE_GH_REPOSITORY\" --signer-workflow \"$FAKE_GH_WORKFLOW\" --format json --bundle \"$FAKE_GH_BUNDLE\")\n[[ $# -eq ${#expected[@]} ]]||exit 40\nfor ((i=0;i<$#;i++));do position=$((i+1));[[ ${!position} == ${expected[i]} ]]||exit 41;done\n[[ $(cat \"$FAKE_GH_BUNDLE\") == valid-bundle ]]||exit 42\ncat \"$FAKE_GH_EVIDENCE\"\n");chmodSync(gh,0o755);
  ...
  writeEvidence();
  const environment={...process.env,PATH:`${fakeBin}:${process.env.PATH}`,FAKE_GH_EVIDENCE:evidencePath,FAKE_GH_INTENT:intentPath,FAKE_GH_REPOSITORY:repository,FAKE_GH_WORKFLOW:workflow,FAKE_GH_BUNDLE:bundlePath};
  const exactGhArgs=["attestation","verify",intentPath,"--repo",repository,"--signer-workflow",workflow,"--format","json","--bundle",bundlePath];
  const run=(overrides:{intent?:any;anchor?:any;bundle?:string;expectedRun?:number;expectedAttempt?:number;expectedBundle?:string;evidenceMutate?:(value:any)=>void}={})=>{if(overrides.anchor){writeFileSync(anchorPath,`${JSON.stringify(overrides.anchor)}\n`)}writeFileSync(bundlePath,overrides.bundle??"valid-bundle");writeEvidence(overrides.intent??intent,overrides.evidenceMutate);return spawnSync("infra/bootstrap/verify-and-create-production-policy-entry.sh",[intentPath,bundlePath,anchorPath,repository,String(overrides.expectedRun??runId),String(overrides.expectedAttempt??runAttempt),overrides.expectedBundle??bundleSha,"root-key",output],{cwd:process.cwd(),env:environment,encoding:"utf8"})};
  return {anchor,intent,run,output,workflow,workflowRef,workflowSha,bundleSha,repository,gh,environment,exactGhArgs};
};

it("creates an entry only from the externally bound attested ceremony",()=>{const x=fixture(),result=x.run();expect(result.status,result.stderr).toBe(0);expect(JSON.parse(readFileSync(x.output,"utf8"))).toMatchObject({repositoryId:"123",bundleSha256:x.bundleSha})},15000);
it("makes the fake gh fail closed on missing, wrong or extra attestation argv",()=>{const x=fixture(),invoke=(args:string[])=>spawnSync(x.gh,args,{env:x.environment});expect(invoke(x.exactGhArgs).status).toBe(0);for(const args of [x.exactGhArgs.slice(0,-2),x.exactGhArgs.map((value,index)=>index===4?"other/repo":value),[...x.exactGhArgs,"extra"]])expect(invoke(args).status).not.toBe(0)});
...
it("rejects every externally bound ceremony identity mismatch",()=>{
  const cases:Array<[string,(x:ReturnType<typeof fixture>)=>Parameters<ReturnType<typeof fixture>["run"]>[0]]>=[
    ...
    ["external expected run id",x=>({expectedRun:x.intent.runId+1})],
    ["external expected run attempt",x=>({expectedAttempt:x.intent.runAttempt+1})],
    ...
    ["external expected policy bundle",_=>({expectedBundle:"7".repeat(64)})],
    ["attestation bundle",_=>({bundle:"wrong-bundle"})],
    ["anchor",x=>({anchor:{...x.anchor,anchorGeneration:x.anchor.anchorGeneration+1}})],
    ["lineage receipt fork",x=>({intent:{...x.intent,previousReceiptDigest:"6".repeat(64)}})],
    ["lineage anchor fork",x=>({intent:{...x.intent,anchorSha256:"5".repeat(64)}})],
    ["lineage generation rollback",x=>({intent:{...x.intent,anchorGeneration:x.intent.anchorGeneration-1}})],
  ];
