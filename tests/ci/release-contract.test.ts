...
describe("release CI contract", () => {
  ...
  it("authoritatively verifies candidate policy from an externally pinned protected checkout",()=>{const workflow=read(".github/workflows/authorize-release.yml"),eligibility="github.event.workflow_run.conclusion == 'success' && github.event.workflow_run.event == 'push' && startsWith(github.event.workflow_run.head_branch, 'v')";expect(workflow.match(new RegExp(eligibility.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),"g"))?.length).toBe(2);for(const text of ["verify-candidate-policy:","environment: production-release-policy-approval",'ref: "${{ github.workflow_sha }}"',"path: trusted",'ref: "${{ github.event.workflow_run.head_sha }}"',"path: candidate",'/usr/bin/node "$GITHUB_WORKSPACE/trusted/scripts/verify-deployment-permissions.mjs"','EXPECTED_POLICY_BUNDLE_SHA256: "${{ vars.PRODUCTION_POLICY_BUNDLE_SHA256 }}"','EXPECTED_POLICY_LAUNCHER_SHA256: "${{ vars.PRODUCTION_POLICY_LAUNCHER_SHA256 }}"',"production-policy-pin-launcher.sh","policyLauncherSha256","verify-production-policy-bundle.mjs","compare/$TRUSTED_WORKFLOW_SHA...$default_head","merge_base_commit","TRUSTED_WORKFLOW_SHA","source-run.json","deployment-policy-evidence.json","verifierSha256","policySha256","policyBundleSha256","attest-build-provenance","needs: verify-candidate-policy"])expect(workflow).toContain(text);expect(workflow).not.toContain('commits/$default_branch" --jq .sha)" = "$TRUSTED_WORKFLOW_SHA');expect(workflow.slice(workflow.indexOf("  authorize:"),workflow.length)).not.toContain("concurrency:");});
  ...
  it("uses an independently pinned launcher and isolated inline Python",()=>{const launcher=read("scripts/production-policy-pin-launcher.sh"),helper=read("scripts/print-production-policy-pin.sh"),workflow=read(".github/workflows/authorize-release.yml");for(const text of ["launcher-sha256","production policy launcher pin mismatch","cat-file blob","STEAM_TOP_POLICY_LAUNCHED=1"])expect(launcher).toContain(text);for(const text of ["PYTHONNOUSERSITE=1","python3 -I -E -S","os.setsid();os.execve","kill -0 -- \"-$active_pid\""])expect(helper).toContain(text);for(const text of ["PRODUCTION_POLICY_LAUNCHER_SHA256","policyLauncherSha256"])expect(workflow).toContain(text)});
