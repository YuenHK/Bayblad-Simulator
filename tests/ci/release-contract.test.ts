...
describe("rollback deletion monotonicity", () => {
  ...
  it("fails closed when the external tombstone ledger advanced", () => {
    ...
    const legacyImport=read("infra/backup/import-legacy-cutover.sh");expect(legacyImport).toContain("CANONICAL_STATE_RESOLVED");expect(legacyImport).toContain("state='legacy-committed'");expect(legacyImport).toContain("final_receipt_signature_b64");expect(legacyImport).toContain('exact(receipt,["purpose","promotionNonce","publicOrigin","systemIdentifier","restoreTargetId","preflightSha256","smokeProbe","verifiedAt"])');expect(legacyImport).toContain("legacy ready digest");
    ...
    expect(read("infra/bootstrap/verify-reaper-health.sh")).toContain("ExecMainStatus");
