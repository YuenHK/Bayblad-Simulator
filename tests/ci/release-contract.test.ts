...
describe("release CI contract", () => {
  ...
  it("gives the fixture operator maintenance access and generates valid role SQL",()=>{const promotion=read("infra/backup/test-promotion-full.sh");for(const sql of ["grant connect on database postgres","grant connect on database %I to %I;","grant %I to %I;","revoke %I from %I;","revoke connect on database %I from %I;"])expect(promotion).toContain(sql);expect(promotion).toContain("select datallowconn from pg_database where datname=current_database()");expect(promotion).not.toContain('select datallowconn||\'|\'||(select environment');});
  it("checks root-only promotion evidence with elevated filesystem access",()=>{const promotion=read("infra/backup/test-promotion-full.sh"),canonical=read("infra/backup/test-canonical-cutover-full.sh");for(const check of ['sudo test -f "$tmp/state/promotion-ready"','sudo test ! -e "$tmp/state-marker/promotion-ready"','sudo find "$tmp/state-recovery"','sudo stat -c \'%u %a\' "$tmp/ledger"'])expect(promotion).toContain(check);expect(promotion).not.toContain('[[ -f $tmp/state/promotion-ready ]]');expect(canonical).toContain('|sudo tee "$tmp/activation.frame" >/dev/null');});
  it("reports the failed durable promotion reconciliation boundary",()=>{const reconcile=read("infra/backup/reconcile-promotion-ready.sh");for(const reason of ["runtime seal","state directory trust","committed promotion outbox","publish ready payload","publish ready checksum"])expect(reconcile).toContain(`reconcile_die "${reason}"`);});
  it("disables target connections through the separate maintenance service",()=>{const promotion=read("infra/backup/promote-restored-target.sh");expect(promotion).toContain('\\! PGSERVICE="$PROMOTE_MAINTENANCE_PGSERVICE"');expect(promotion).toContain("select not datallowconn as connections_disabled from pg_database where datname=current_database() \\gset");expect(promotion).not.toContain("select format('alter database %I allow_connections false',current_database()) \\gexec");expect(promotion).not.toContain("export PGSERVICE=");});
  it("rejects application roles with direct or nested role memberships before promotion",()=>{const promotion=read("infra/backup/promote-restored-target.sh");expect(promotion).toContain("with recursive memberships(roleid)");expect(promotion).toContain("from pg_auth_members where member=(select oid from pg_roles where rolname=:'role')");expect(promotion).toContain('die "application role memberships forbidden"');});
  it("keeps canonical installer status output out of the GitHub environment file",()=>{const fixture=read("scripts/install-canonical-ci-fixture.sh"),workflow=read(".github/workflows/ci.yml");expect(fixture).toContain('install_stdout="$RUNNER_TEMP/canonical-bootstrap-install.stdout"');expect(fixture).toContain('>"$install_stdout" 2>"$install_trace"');expect(workflow).toContain('./scripts/install-canonical-ci-fixture.sh > "$RUNNER_TEMP/canonical-fixture.env"');});
  it("uses a dedicated cluster and non-owner application identity only after migration and pristine claim",()=>{const deploy=read("scripts/deploy-production.sh"),runtime=read("scripts/create-runtime-files-manifest.mjs"),compose=read("compose.canonical-app.yaml"),provision=read("scripts/provision-app-role.sql"),ci=read(".github/workflows/ci.yml");expect(runtime).toContain("compose.canonical-app.yaml");expect(runtime).toContain("infra/backup/trusted-ledger-cli.sha256");expect(runtime).toContain('path === "apps/server/dist/admin/deletion-ledger-cli.js"');expect(compose).toContain("!override");expect(deploy.indexOf('run --rm migration')).toBeLessThan(deploy.indexOf("provision-app-role.sql"));expect(deploy.indexOf("claim-first-installation.sh")).toBeLessThan(deploy.indexOf("provision-app-role.sql"));for(const text of ["pg_advisory_xact_lock","dedicated PostgreSQL cluster required","datname NOT IN(current_database(),'postgres','template0','template1')","REVOKE CONNECT ON DATABASE postgres FROM PUBLIC,steam_top_app","has_database_privilege('steam_top_app',d.datname,'CONNECT')","REVOKE CREATE ON SCHEMA public FROM PUBLIC","REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA public"])expect(provision).toContain(text);expect(provision).not.toContain("REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA public,restore_control");expect(provision.indexOf("dedicated PostgreSQL cluster required")).toBeLessThan(provision.indexOf("CREATE ROLE steam_top_app"));for(const text of ["APP_DATABASE_URL","compose.json","c.services.db.ports","test ! -e /run/secrets/database_url"])expect(ci).toContain(text);});
...
describe("rollback deletion monotonicity", () => {
  ...
  it("fails closed when the external tombstone ledger advanced", () => {
    ...
    expect(read(".github/workflows/db.yml")).toContain('/opt/steam-top/releases/$RUNTIME_INSTALL_MANIFEST_SHA256/infra/backup/test-canonical-cutover-full.sh');
    const canonical = read("infra/backup/test-canonical-cutover-full.sh");
    expect(read(".github/workflows/ci.yml")).toContain('/opt/steam-top/releases/$RUNTIME_INSTALL_MANIFEST_SHA256/infra/backup/test-canonical-cutover-full.sh');
