...
describe("release CI contract", () => {
  ...
  it("authoritatively verifies candidate policy from the protected default-branch checkout",()=>{const workflow=read(".github/workflows/authorize-release.yml");for(const text of ["verify-candidate-policy:",'ref: "${{ github.workflow_sha }}"',"path: trusted",'ref: "${{ github.event.workflow_run.head_sha }}"',"path: candidate",'/usr/bin/node "$GITHUB_WORKSPACE/trusted/scripts/verify-deployment-permissions.mjs"',"TRUSTED_WORKFLOW_SHA","source-run.json","deployment-policy-evidence.json","verifierSha256","policySha256","attest-build-provenance","needs: verify-candidate-policy"])expect(workflow).toContain(text);expect(workflow.slice(workflow.indexOf("  authorize:"),workflow.length)).not.toContain("concurrency:");});
