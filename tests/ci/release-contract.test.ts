...
describe("release CI contract", () => {
  ...
  it("uses a unique per-run environment and error-only fallback when source binding is absent",()=>{const ci=read(".github/workflows/ci.yml"),reconcile=read(".github/workflows/reconcile-production-e2e.yml");expect(ci).toContain('production-first-deploy-e2e-${{ github.run_id }}-${{ github.run_attempt }}');expect(reconcile).toContain('if [[ -f $binding ]]');expect(reconcile).toContain('deployments?environment=$exact_environment');expect(reconcile).toContain("fallback candidates are error-only");});
  it("bounds and orders only fully bound orphan fallback candidates",()=>{const reconcile=read(".github/workflows/reconcile-production-e2e.yml");for(const text of ["FALLBACK_CANDIDATE_CAP=8","authorizationWorkflowRef","authorizationWorkflowSha","sourceWorkflow","sourceEvent","created_at","fallback-overflow","fallback candidate overflow conflict"])expect(reconcile).toContain(text);expect(reconcile).toContain(".sort(");});
