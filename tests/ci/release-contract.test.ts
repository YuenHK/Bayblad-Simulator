...
describe("release CI contract", () => {
  ...
  it("terminates every generated role grant and revoke before piping it to psql",()=>{const promotion=read("infra/backup/test-promotion-full.sh");for(const sql of ["grant connect on database %I to %I;","grant %I to %I;","revoke %I from %I;","revoke connect on database %I from %I;"])expect(promotion).toContain(sql);});
