...
describe("release CI contract", () => {
  ...
  it("rotates policy only from trusted code with protected CAS",()=>{const launcher=read("scripts/production-policy-pin-launcher.sh"),helper=read("scripts/print-production-policy-pin.sh"),workflow=read(".github/workflows/rotate-production-policy.yml");for(const text of ["usage: %s <repository> <full-40-hex-oid>","cat-file blob"])expect(launcher).toContain(text);for(const text of ["PYTHONNOUSERSITE=1","python3 -I -E -S","kill -0 -- \"-$active_pid\""])expect(helper).toContain(text);for(const text of ["production-policy-rotation-authority","PRODUCTION_POLICY_ROOT_ALLOWED_SIGNERS","trusted/scripts/verify-production-policy-bundle.mjs","compare/$REVIEWED_COMMIT...$default_head","production-policy-rotation-intent","authorized:false"])expect(workflow).toContain(text);expect(workflow).not.toContain("production-policy-pin-launcher.sh")});
