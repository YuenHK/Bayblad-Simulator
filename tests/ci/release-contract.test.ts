...
describe("rollback deletion monotonicity", () => {
  ...
  it("fails closed when the external tombstone ledger advanced", () => {
    ...
    const ciWorkflow=read(".github/workflows/ci.yml"),compose=read("compose.yaml");expect(ciWorkflow).toContain("production-first-deploy-e2e:");expect(ciWorkflow).toContain("needs: release-host-core-integration");expect(ciWorkflow).toContain("DEPLOYMENT_PURPOSE=production node scripts/create-pending-deployment.mjs");expect(ciWorkflow).toContain("--install-systemd --initialize-first-deploy");expect(ciWorkflow).toContain("deploy-release.sh --prepared-authorization");expect(ciWorkflow).toContain("activate-production-state.sh");expect(ciWorkflow).toContain("verify-reaper-health.sh --require-active-runtime");expect(ciWorkflow).toContain("^consumed $PRODUCTION_E2E_NONCE");expect(ciWorkflow).toContain("generation=2");expect(ciWorkflow).toContain("down -v --remove-orphans");expect(compose).toContain('127.0.0.1:${POSTGRES_HOST_PORT:-15432}:5432');
