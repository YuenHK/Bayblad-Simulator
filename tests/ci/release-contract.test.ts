...
describe("release CI contract", () => {
  ...
  it("preflights every fully bound orphan before any idempotent error write",()=>{const reconcile=read(".github/workflows/reconcile-production-e2e.yml");for(const text of ["authorizationWorkflowRef","authorizationWorkflowSha","sourceWorkflow","sourceEvent","created_at","fallback-statuses","plan.conflicts","plan.pending","already-error",'outcome:"no-deployment"'])expect(reconcile).toContain(text);expect(reconcile).toContain(".sort(");expect(reconcile).not.toContain("FALLBACK_CANDIDATE_CAP");expect(reconcile.indexOf("plan.conflicts")).toBeLessThan(reconcile.indexOf('gh api --method POST "repos/$GITHUB_REPOSITORY/deployments/$id/statuses"'));});
  it("enforces the exact protected Deployment writer policy",()=>{const policy=read("scripts/verify-deployment-permissions.mjs"),ci=read(".github/workflows/ci.yml");for(const text of ["authorize-release.yml","reconcile-deployment.yml","reconcile-production-e2e.yml","record-deployment.yml","production-first-deploy-e2e","release-host-core-integration","unauthorized deployments:write","workflow_call guard"])expect(policy).toContain(text);expect(ci).toContain("node scripts/verify-deployment-permissions.mjs");});
