...
describe("release CI contract", () => {
  ...
  it("reconciles orphaned production E2E deployments without racing a live finalizer",()=>{const workflow=read(".github/workflows/reconcile-production-e2e.yml"),ci=read(".github/workflows/ci.yml"),job=ci.slice(ci.indexOf("  production-first-deploy-e2e:"),ci.indexOf("  release-images:"));expect(workflow).toContain("workflow_run:");expect(workflow).toContain("run_attempt");expect(workflow).toContain("authorizationRunId");expect(workflow).toContain("authorizationRunAttempt");expect(workflow).toContain("workflow_run fires only after the source run ended");expect(workflow).toContain('state:"error"');const create=job.indexOf('deployments" --input pending-production.json'),persist=job.indexOf("PRODUCTION_E2E_DEPLOYMENT_ID=%s"),fallible=job.indexOf("create-bootstrap-authorization.mjs");expect(create).toBeGreaterThan(0);expect(create).toBeLessThan(persist);expect(persist).toBeLessThan(fallible);});
  it("uses a non-owner application identity only after migration and pristine claim",()=>{const deploy=read("scripts/deploy-production.sh"),runtime=read("scripts/create-runtime-files-manifest.mjs"),compose=read("compose.canonical-app.yaml"),ci=read(".github/workflows/ci.yml");expect(runtime).toContain("compose.canonical-app.yaml");expect(compose).toContain("app_database_url");expect(deploy.indexOf('run --rm migration')).toBeLessThan(deploy.indexOf("create role steam_top_app"));expect(deploy.indexOf("claim-first-installation.sh")).toBeLessThan(deploy.indexOf("create role steam_top_app"));expect(deploy).toContain("nosuperuser nocreatedb nocreaterole noinherit noreplication");expect(ci).toContain("APP_DATABASE_URL");expect(ci).toContain("not rolsuper");});
