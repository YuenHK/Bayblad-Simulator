...
describe("release CI contract", () => {
  ...
  it("reconciles orphaned production E2E deployments without racing a live finalizer",()=>{const workflow=read(".github/workflows/reconcile-production-e2e.yml"),ci=read(".github/workflows/ci.yml"),job=ci.slice(ci.indexOf("  production-first-deploy-e2e:"),ci.indexOf("  release-images:"));for(const text of ["workflow_run:","run_attempt","authorizationRunId","authorizationRunAttempt","--paginate --slurp","candidate-run.json","reconciliation-receipts.jsonl","attest-build-provenance",'state:"error"',"terminal status conflict requires manual review"])expect(workflow).toContain(text);expect(workflow).not.toContain('state:"success"');const create=job.indexOf('deployments" --input pending-production.json'),persist=job.indexOf("PRODUCTION_E2E_DEPLOYMENT_ID=%s"),fallible=job.indexOf("create-bootstrap-authorization.mjs");expect(create).toBeGreaterThan(0);expect(create).toBeLessThan(persist);expect(persist).toBeLessThan(fallible);});
