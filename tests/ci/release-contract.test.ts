...
describe("rollback deletion monotonicity", () => {
  ...
  it("fails closed when the external tombstone ledger advanced", () => {
    ...
    const recordWorkflow=read(".github/workflows/record-deployment.yml");expect(recordWorkflow.indexOf('verify-reaper-health.sh"')).toBeLessThan(recordWorkflow.indexOf("deploy-release.sh"));expect(recordWorkflow.indexOf("activate-production-state.sh")).toBeLessThan(recordWorkflow.indexOf("verify-reaper-health.sh --require-active-runtime"));const firstDeploy=read("scripts/deploy-production.sh");expect(firstDeploy.indexOf('up -d --wait db')).toBeLessThan(firstDeploy.indexOf('run --rm migration'));expect(firstDeploy.indexOf('run --rm migration')).toBeLessThan(firstDeploy.lastIndexOf('up -d --wait'));
