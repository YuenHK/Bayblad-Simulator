...
describe("rollback deletion monotonicity", () => {
  ...
  it("fails closed when the external tombstone ledger advanced", () => {
    ...
    for(const column of ["deadline_at","lease_owner","lease_generation","ledger_hash","ready_sha256","preflight_sha256","smoke_evidence_payload_b64","smoke_evidence_sha256","final_receipt_sha256","final_receipt_payload_b64","legacy_ready_payload_b64","legacy_ready_sha_file_b64","legacy_preflight_signature_b64"])expect(cutoverMigration).toContain(column);
    ...
    expect(read("infra/bootstrap/verify-reaper-health.sh")).toContain("ExecMainStatus");expect(read("infra/bootstrap/verify-reaper-health.sh")).toContain("not-applicable-clean-host");expect(read("infra/systemd/steam-top-cutover-reaper.service")).toContain("ConditionPathExists=/opt/steam-top/current");
    expect(read(".github/workflows/record-deployment.yml")).toContain("sudo /opt/steam-top-bootstrap/verify-reaper-health.sh");
    const recordWorkflow=read(".github/workflows/record-deployment.yml");expect(recordWorkflow.indexOf('verify-reaper-health.sh"')).toBeLessThan(recordWorkflow.indexOf("deploy-release.sh"));expect(recordWorkflow.indexOf("deploy-release.sh")).toBeLessThan(recordWorkflow.indexOf("verify-reaper-health.sh --require-active-runtime"));
