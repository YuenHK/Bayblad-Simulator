...
describe("release CI contract", () => {
  ...
  it("computes a rotation pin only from a disposable clean checkout",()=>{const helper=read("scripts/print-production-policy-pin.sh"),verifier=read("scripts/verify-production-policy-bundle.mjs");for(const text of ["mktemp -d","git clone --no-local","temporary/repository","rev-parse HEAD"])expect(helper).toContain(text);for(const text of ["write-tree","HEAD^{tree}","diff-files","--ignored=matching","cat-file","entry.oid"])expect(verifier).toContain(text);});
