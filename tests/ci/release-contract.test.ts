...
describe("release CI contract", () => {
  ...
  it("computes a rotation pin only from an independent detached reviewed checkout",()=>{const helper=read("scripts/print-production-policy-pin.sh"),verifier=read("scripts/verify-production-policy-bundle.mjs");for(const text of ["reviewed=${1:-HEAD}","git clone --no-hardlinks --no-checkout","objects/info/alternates","checkout --detach","source_commit","rev-parse HEAD"])expect(helper).toContain(text);for(const text of ["write-tree","HEAD^{tree}","--really-refresh","diff-files","--ignored=matching","cat-file","entry.oid","TextDecoder","normalize(\"NFC\")"])expect(verifier).toContain(text);});
