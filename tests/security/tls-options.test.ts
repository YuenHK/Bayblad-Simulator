...
describe("security test TLS trust", () => {
  ...
  it("loads an explicit local CA and keeps verification enabled", () => {
    ...
    expect(securityTlsOptions({ SECURITY_HTTPS_ORIGIN: "https://localhost", SECURITY_TLS_CA_FILE: "/run/caddy/pki/ca.crt", SECURITY_TLS_INSECURE: "true" }, read)).toEqual({
  ...
  it("allows insecure TLS only with exact true on a loopback origin", () => {
    expect(securityTlsOptions({ SECURITY_HTTPS_ORIGIN: "https://127.0.0.1", SECURITY_TLS_INSECURE: "true" })).toEqual({ rejectUnauthorized: false });
    expect(() => securityTlsOptions({ SECURITY_HTTPS_ORIGIN: "https://tops.school.example", SECURITY_TLS_INSECURE: "true" })).toThrow("loopback");
    expect(() => securityTlsOptions({ SECURITY_HTTPS_ORIGIN: "https://localhost", SECURITY_TLS_INSECURE: "TRUE" })).toThrow("true or false");
  });
