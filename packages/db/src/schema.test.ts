...
describe("persistent PostgreSQL schema", () => {
  ...
  it("commits generated SQL containing constraints, FKs, indexes and no credential", () => {
    ...
    const migrationFiles = readdirSync(migrationDirectory)
      ...
      .sort();
    expect(migrationFiles).toEqual(["0000_steam_top_pre_first_deploy.sql", "0001_cutover_state_machine.sql", "0002_platform_installation.sql", "0003_postgresql_catalog_array_compatibility.sql"]);
    ...
    expect(sql.toLowerCase()).not.toContain("EXAMPLE_PASSWORD_DO_NOT_USE");
