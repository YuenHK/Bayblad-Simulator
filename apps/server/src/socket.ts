...
export class RealtimeGateway {
  ...
  async #enqueueCommand(socket: Socket, session: Session, event: V1CommandEvent, receivedAtMs: number): Promise<void> {
    ...
    const referencedRoom = "roomId" in event && typeof event.roomId === "string" ? (this.#rooms.resolveRoomReference(event.roomId) ?? event.roomId) : undefined;
    const roomTail = referencedRoom ? (this.#roomCommandTails.get(referencedRoom) ?? Promise.resolve()) : Promise.resolve();
    const execution = Promise.all([session.commandTail, roomTail]).then(async () => {
      if (session.commandsStopped) throw Object.assign(new Error("COMMAND_FAILED"), { code: "COMMAND_FAILED" });
    ...
    if (referencedRoom) {
      const settled = execution.then(() => undefined, () => undefined).finally(() => { if (this.#roomCommandTails.get(referencedRoom) === settled) this.#roomCommandTails.delete(referencedRoom); });
      this.#roomCommandTails.set(referencedRoom, settled);
    }
    try {
      await execution;
  ...
  async #startMatchIfReady(roomId: string): Promise<void> {
    ...
    try {
      ...
      await this.#transitionRoomPhase(roomId, "launch", "launch", true);
      ...
      try { await this.#transitionRoomPhase(roomId, "waiting", "cancel", true); } catch (rollbackError) { this.#logError(rollbackError); }
      this.#broadcastRoom(roomId);
      this.#broadcastLobby();
      throw error;
    }
  ...
  #flushLaunch(roomId: string, match: MatchState): void {
    ...
    if (match.launches.size === 2 && !match.simulating) void this.#simulate(roomId, match).catch(this.#logError);
  }
  ...
  async #simulate(roomId: string, match: MatchState): Promise<void> {
    ...
    try {
      await this.#transitionRoomPhase(roomId, "battle", "battle");
      this.#broadcastRoom(roomId);
      this.#broadcastLobby();
      ...
      await this.#transitionRoomPhase(roomId, "result", "result");
      this.#broadcastRoom(roomId);
      this.#broadcastLobby();
      ...
      if (completedScore) {
        ...
        try {
          ...
          try { await this.#transitionRoomPhase(roomId, "waiting", "finish"); }
      ...
      if (!match.officiallyCompleted && !(error instanceof Error && (error.name === "AbortError" || error.name === "MatchPersistenceTerminalError" || error.name === "RoundPersistenceTerminalError"))) {
        ...
        if (this.#matches.get(roomId) === match && match.generation === generation) {
          ...
          match.latestRoundFinished = null;
          try {
            await this.#transitionRoomPhase(roomId, "launch", "retry-round");
            this.#broadcastRoom(roomId);
            this.#broadcastLobby();
            this.#scheduleRound(roomId, match);
          } catch (recoveryError) {
            this.#logError(recoveryError);
            try { await this.#transitionRoomPhase(roomId, "waiting", "cancel"); }
  ...
  async #cancelForAttemptLimit(roomId: string, match: MatchState): Promise<void> {
    this.#emitToRoom(roomId, {
  ...
  #scheduleTerminalRecovery(roomId: string): void {
    ...
    const timer = setTimeout(() => {
      ...
      void this.#transitionRoomPhase(roomId, "waiting", "cancel").then(() => { this.#broadcastRoom(roomId); this.#broadcastLobby(); }, (error: unknown) => { this.#logError(error); this.#scheduleTerminalRecovery(roomId); });
    }, 1_000);
  ...
  async #transitionRoomPhase(roomId: string, phase: "waiting" | "launch" | "battle" | "result", transition: import("./rooms/room-service").PersistedRoomTransition, roomLocked = false): Promise<void> {
    if (!roomLocked) {
      return this.#runInRoomTail(roomId, () => this.#transitionRoomPhase(roomId, phase, transition, true));
    }
    const reservation = this.#rooms.reservePersistedTransition(roomId, transition);
    const expectedRevision = reservation.expectedRevision;
    ...
    if (this.#roomRecordRepository && this.#roomProjections.usesDurableStore && !databaseAuthoritative) throw new Error("ROOM_AUTHORITATIVE_TRANSITION_UNAVAILABLE");
    if (databaseAuthoritative) {
      let lastError: unknown;
      for (let attempt = 0; attempt < 3; attempt += 1) { try { await this.#roomRecordRepository!.transitionPhaseWithProjection!(roomId, expectedRevision, payload, new Date(this.#now())); lastError = undefined; break; } catch (error) { lastError = error; } }
  ...
  async #projectRoomPhase(roomId: string, phase: "waiting" | "launch" | "battle" | "result", reservedRevision?: number): Promise<void> {
    if (!this.#roomRecordRepository) return;
    const revision = reservedRevision ?? (this.#rooms.get(roomId)?.revision ?? Number.MAX_SAFE_INTEGER);
    if (this.#roomProjections.usesDurableStore) {
      let lastError: unknown;
      for (let attempt = 0; attempt < 3; attempt += 1) {
        try {
          await this.#roomProjections.enqueueProjection({ roomId, revision, payload: { phase, firstBattleAt: this.#matches.get(roomId)?.startedAt.toISOString() ?? null, closedAt: null } });
          return;
        } catch (error) { lastError = error; if (attempt < 2) await new Promise<void>((resolve) => setTimeout(resolve, 0)); }
      }
      throw lastError;
