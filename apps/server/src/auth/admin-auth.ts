...
export const auditInputSchema = z.object({
  adminUserId: z.string().min(1).max(128).optional(), adminSessionId: z.string().min(1).max(128).optional(),
  action: z.string().min(1).max(128).regex(/^[a-z0-9._-]+$/), outcome: z.enum(["success", "failure", "denied"]),
  ip: z.string().max(64).optional(), userAgent: z.string().max(512).optional(), details: z.record(z.string(), z.unknown()).optional(),
}).strict().superRefine((value, context) => {
  if (Buffer.byteLength(JSON.stringify(value), "utf8") > 16_384) context.addIssue({ code: "custom", message: "AUDIT_PAYLOAD_TOO_LARGE" });
});
export async function durableAudit(store: AdminStore, input: AuditInput): Promise<void> {
  const parsed = auditInputSchema.parse(input) as AuditInput;
  await (store.queueAudit?.bind(store) ?? store.audit.bind(store))(parsed);
}
...
export class InMemoryAdminStore implements AdminStore {
  ...
  async admitLoginAttempt(keys: { accountHash: string; clientHash: string }, now: Date) { return !this.#loginLimits.isLocked(keys.accountHash, keys.clientHash, now.getTime()); }
  async recordLoginFailureAndStatus(keys: { accountHash: string; clientHash: string }, now: Date) { if (this.#loginLimits.isLocked(keys.accountHash, keys.clientHash, now.getTime())) return true; this.#loginLimits.fail(keys.accountHash, keys.clientHash, now.getTime()); return false; }
  ...
  async createReauthGrant(input: { tokenHash: string; adminUserId: string; adminSessionId: string; purpose: string; expiresAt: Date }) { const active = [...this.#users.values()].some((user) => user.id === input.adminUserId && user.active); const now = input.expiresAt.getTime() - 5 * 60_000; const session = [...this.#sessions.values()].find((row) => row.id === input.adminSessionId && row.adminUserId === input.adminUserId && !row.revokedAt && !row.archivedAt && now < row.absoluteExpiresAt.getTime() && now - row.lastSeenAt.getTime() < ADMIN_IDLE_MS); if (!active || !session) throw new Error("ADMIN_INACTIVE"); this.#grants.set(input.tokenHash, { ...input, consumed: false }); }
...
export class AdminAuthService {
  ...
  async verifyPassword(username: string, password: string): Promise<boolean> { const user = await this.store.findUser(username.trim()); const target = user?.passwordHash ?? await this.#dummyHash; try { return Boolean(user?.active && await this.#withArgon(() => verify(target, password))); } catch (error) { this.report("argon.verify", error); throw error; } }
...
export async function authenticateAdminMutation(request: FastifyRequest, reply: FastifyReply, auth: AdminAuthService, resolver: AdminClientResolver = directClient) {
  ...
  try {
    ...
    if (!raw || !auth.csrfMatches(raw, supplied, current.session)) { const resolved = diagnostics(request, resolver); await durableAudit(auth.store, { adminUserId: current.user.id, adminSessionId: current.session.id, action: "admin.csrf", outcome: "denied", ...(resolved.ip ? { ip: resolved.ip } : {}), ...(resolved.userAgent ? { userAgent: resolved.userAgent } : {}) }); reply.code(403).send({ error: "CSRF_REJECTED" }); return null; }
