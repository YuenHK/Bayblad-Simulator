...
export class AdminAuthService {
  ...
  async login(username: string, password: string, diagnostics: { clientKey: string; ip?: string; userAgent?: string }) {
    ...
    if (!await this.store.admitLoginAttempt({ accountHash, clientHash }, now)) { await this.store.audit({ action: "admin.login.locked", outcome: "denied", ...auditDiagnostics, details: { accountHash, clientHash } }); return { status: "locked" as const }; }
    const user = await this.store.findUser(username.trim()); const valid = await this.verifyPassword(username, password);
    if (!user || !valid) { const locked = await this.store.recordLoginFailureAndStatus({ accountHash, clientHash }, now); await this.store.audit({ ...(user ? { adminUserId: user.id } : {}), action: "admin.login", outcome: "failure", ...auditDiagnostics, details: { accountHash, clientHash, ...(!user ? { unknownAccount: true } : {}) } }); return locked ? { status: "locked" as const } : { status: "invalid" as const }; }
