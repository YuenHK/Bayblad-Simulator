...
export class InMemoryAdminStore implements AdminStore {
  ...
  readonly #grants = new Map<string, { adminUserId: string; adminSessionId: string; purpose: string; expiresAt: Date; consumed: boolean }>();
  ...
  async consumeReauthGrant(input: { tokenHash: string; adminUserId: string; adminSessionId: string; purpose: string; now: Date }) { const row = this.#grants.get(input.tokenHash); if (!row || row.consumed || input.now.getTime() >= row.expiresAt.getTime() || row.adminUserId !== input.adminUserId || row.adminSessionId !== input.adminSessionId || row.purpose !== input.purpose) return false; row.consumed = true; return true; }
}
...
export class AdminAuthService {
  ...
  constructor(readonly store: AdminStore, options: Readonly<{ now?: () => Date; allowedOrigins: readonly string[]; secureCookies?: boolean; tokenFactory?: () => string; csrfSecret?: Buffer; csrfKeyId?: string }>) {
  ...
  async login(username: string, password: string, diagnostics: { clientKey: string; ip?: string; userAgent?: string }) {
    ...
    if (!this.#limiter.reserve(account, diagnostics.clientKey, now.getTime()) || (this.store.loginAllowed && !await this.store.loginAllowed({ accountHash, clientHash }, now))) { await this.store.audit({ action: "admin.login.locked", outcome: "denied", ...auditDiagnostics, details: { accountHash, clientHash } }); return { status: "locked" as const }; }
    ...
    this.#limiter.success(account, diagnostics.clientKey); const raw = this.#tokens(); if (Buffer.from(raw, "base64url").length < 32) throw new Error("ADMIN_TOKEN_FACTORY_TOO_SHORT"); const csrf = csrfForSession(this.#csrfSecret, raw, this.#csrfKeyId);
...
export async function authenticateAdminRead(request: FastifyRequest, reply: FastifyReply, auth: AdminAuthService) {
  ...
  try { const current = await auth.authenticate(request.cookies[ADMIN_COOKIE_NAME]); if (!current) reply.code(401).send({ error: "UNAUTHORIZED" }); return current; } catch { reply.code(503).send({ error: "ADMIN_STORE_UNAVAILABLE" }); return null; }
}
