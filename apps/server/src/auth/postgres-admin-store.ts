...
export class PostgresAdminStore implements AdminStore {
  ...
  async pruneExpiredSessions(now: Date, limit: number) { if (!Number.isSafeInteger(limit) || limit < 1 || limit > 5_000) throw new TypeError("INVALID_PRUNE_LIMIT"); const idle = new Date(now.getTime() - ADMIN_IDLE_MS).toISOString(); const stamp = now.toISOString(); const rows = await this.db.execute(sql`with expired as (select id from admin_sessions where archived_at is null and (expires_at <= ${stamp}::timestamptz or last_seen_at <= ${idle}::timestamptz) order by expires_at limit ${limit} for update skip locked) update admin_sessions s set revoked_at=${stamp}::timestamptz, archived_at=${stamp}::timestamptz, updated_at=${stamp}::timestamptz, token_hash=encode(digest(s.token_hash || ${stamp},'sha256'),'hex'), csrf_token_hash=encode(digest(s.csrf_token_hash || ${stamp},'sha256'),'hex') from expired where s.id=expired.id returning s.id`); await this.db.delete(adminReauthGrants).where(lt(adminReauthGrants.expiresAt, new Date(now.getTime()-7*86_400_000))); await this.db.delete(adminLoginLimits).where(lt(adminLoginLimits.updatedAt,new Date(now.getTime()-30*60_000))); await this.db.delete(adminSessions).where(and(lt(adminSessions.archivedAt,new Date(now.getTime()-30*86_400_000)),sql`${adminSessions.archivedAt} is not null`)); return rows.length; }
