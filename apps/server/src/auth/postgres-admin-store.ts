...
export class PostgresAdminStore implements AdminStore {
  ...
  async loginAllowed(keys: { accountHash: string; clientHash: string }, now: Date) {
    return this.db.transaction(async (tx) => {
      await tx.execute(sql`select pg_advisory_xact_lock(1937002748)`);
      const lockStart = new Date(now.getTime() - 15 * 60_000); const globalStart = new Date(now.getTime() - 60_000);
      const [row] = await tx.select({ account: count(sql`case when ${adminAudit.action} = 'admin.login.reserved' and ${adminAudit.details}->>'accountHash' = ${keys.accountHash} and ${adminAudit.createdAt} > coalesce((select max(reset.created_at) from admin_audit reset where reset.action = 'admin.login' and reset.outcome = 'success' and reset.details->>'accountHash' = ${keys.accountHash}), ${lockStart}) then 1 end`), client: count(sql`case when ${adminAudit.action} = 'admin.login.reserved' and ${adminAudit.details}->>'clientHash' = ${keys.clientHash} and ${adminAudit.createdAt} >= ${lockStart} then 1 end`), global: count(sql`case when ${adminAudit.createdAt} >= ${globalStart} and ${adminAudit.action} = 'admin.login.reserved' then 1 end`) }).from(adminAudit).where(gte(adminAudit.createdAt, lockStart));
      if ((row?.account ?? 0) >= 5 || (row?.client ?? 0) >= 20 || (row?.global ?? 0) >= 200) return false;
      await tx.insert(adminAudit).values({ action: "admin.login.reserved", outcome: "denied", details: keys, createdAt: now }); return true;
    });
  }
  async createReauthGrant(input: { tokenHash: string; adminUserId: string; adminSessionId: string; purpose: string; expiresAt: Date }) { await this.db.insert(adminAudit).values({ adminUserId: input.adminUserId, adminSessionId: input.adminSessionId, action: "admin.reauth.grant", targetType: "reauth_grant", targetId: input.tokenHash, outcome: "success", details: { purpose: input.purpose, expiresAt: input.expiresAt.toISOString() } }); }
  async consumeReauthGrant(input: { tokenHash: string; adminUserId: string; adminSessionId: string; purpose: string; now: Date }) {
    return this.db.transaction(async (tx) => { await tx.execute(sql`select pg_advisory_xact_lock(hashtextextended(${input.tokenHash}, 1937002747))`); const [grant] = await tx.select().from(adminAudit).where(and(eq(adminAudit.action, "admin.reauth.grant"), eq(adminAudit.targetId, input.tokenHash), eq(adminAudit.adminUserId, input.adminUserId), eq(adminAudit.adminSessionId, input.adminSessionId))).limit(1); if (!grant || grant.details.purpose !== input.purpose || typeof grant.details.expiresAt !== "string" || input.now.getTime() >= new Date(grant.details.expiresAt).getTime()) return false; const [used] = await tx.select({ value: count() }).from(adminAudit).where(and(eq(adminAudit.action, "admin.reauth.consume"), eq(adminAudit.targetId, input.tokenHash))); if ((used?.value ?? 0) > 0) return false; await tx.insert(adminAudit).values({ adminUserId: input.adminUserId, adminSessionId: input.adminSessionId, action: "admin.reauth.consume", targetType: "reauth_grant", targetId: input.tokenHash, outcome: "success", details: { purpose: input.purpose } }); return true; });
  }
}
