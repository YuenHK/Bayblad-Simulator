...
async function main() {
  ...
  try {
    const sources = ["0000_steam_top_pre_first_deploy.sql", "0001_cutover_state_machine.sql", "0002_platform_installation.sql", "0003_postgresql_catalog_array_compatibility.sql", "0004_scoped_readiness_acl.sql"].map((name) => readFileSync(`/app/drizzle/${name}`, "utf8"));
