...
export class RoomProjectionCoordinator {
  ...
  #claimPump: Promise<void> | null = null;
  #closed = false;
  ...
  async pump(now = new Date(), limit = 25): Promise<void> {
    ...
    if (this.#claimPump) return this.#claimPump;
    const pump = this.#pumpOnce(now, limit).finally(() => { if (this.#claimPump === pump) this.#claimPump = null; });
    this.#claimPump = pump;
    return pump;
  }
  async #pumpOnce(now: Date, limit: number): Promise<void> {
    await this.#store!.pruneDead?.(now, 1_000);
    const claims = await this.#store!.claimDue(limit, now);
    if (this.#closed) {
      await Promise.allSettled(claims.map((claim) => this.#store!.fail(claim, "ROOM_PROJECTION_SHUTDOWN", now)));
      return;
    }
  ...
  async close(): Promise<void> {
    ...
    this.#entries.clear();
    const results = await Promise.allSettled([...this.#pendingEnqueues, ...(this.#claimPump ? [this.#claimPump] : []), ...this.#runningClaims]);
