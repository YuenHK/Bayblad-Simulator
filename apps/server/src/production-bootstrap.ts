...
export async function startProductionServer(
  ...
  const app = buildApp({ ...options, ...(listen.reportStartupStage ? { reportStartupStage: listen.reportStartupStage } : {}), ...createProductionRecordRepositories(client), analyticsService: options.analyticsService ?? createProductionAnalytics(client), exportDataSource: options.exportDataSource ?? createProductionExportDataSource(client), deletionStore: new PostgresDeletionStore(client, 1_000, new FileDeletionLedger(ledgerPath)), adminRecordsSource: options.adminRecordsSource ?? new PostgresAdminRecordsSource(client.sql), deploymentProbeSource: options.deploymentProbeSource ?? new PostgresDeploymentProbeSource({ unsafe: async (query, params) => await client.sql.unsafe(query, params as never[]) as readonly Record<string, unknown>[] }), platformSettingsStore: options.platformSettingsStore ?? new PostgresPlatformSettingsStore(client), adminCommandStore: options.adminCommandStore ?? new PostgresAdminCommandStore(client), requireAuthorityLease: true });
