...
beforeAll(async () => {
  ...
  client = createDatabaseClient({ url: databaseUrl, ssl: false, allowInsecure: true, maxConnections: 1 });
  ...
  const first = randomUUID(), second = randomUUID(), star = randomUUID(), circle = randomUUID();
  await client.sql`insert into identities(id,status,display_name,class_name,anonymous_device_id) values (${first},'iclass','1A-01','1A',${randomUUID()}),(${second},'guest','訪客-ABCD',null,${randomUUID()})`;
  ...
  await addDesign(star, first, "star", "2026-07-31T16:10:00Z"); await addDesign(circle, second, "circle", "2026-08-31T16:10:00Z");
...
it.skipIf(!databaseUrl)("returns only parameter groups backed by at least ten completed matches", async () => {
  ...
  expect(rows).toHaveLength(6); // three layer positions for each of two designs
  expect(rows.every((row) => row.sampleSize === 10)).toBe(true);
  expect(rows.find((row) => row.parameters.shape === "star")).toMatchObject({ averageScore: 2, winRate: 1, opponentAverageStrength: 0, launchGrades: { Perfect: 20 } });
}, 30_000);
...
it.skipIf(!databaseUrl)("persists and reuses a bounded materialized summary", async () => {
  ...
  const service = new AnalyticsService(cache, async (filters) => { executions++; return usageAnalytics(client.db, filters); }, async (filters) => parameterPerformance(client.db, filters), () => new Date("2026-09-30T16:00:00Z"));
