...
export function createProductionAnalytics(client: DatabaseClient): AnalyticsService {
  ...
  const consistent=<T>(operation:()=>Promise<T>)=>{const executor=cache.currentExecutor();if(!("options" in executor))Object.defineProperty(executor,"options",{value:client.sql.options});return snapshotDb.run(drizzle(executor as never,{schema}),operation);};
