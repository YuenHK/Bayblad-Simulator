...
export function createProductionAnalytics(client: DatabaseClient): AnalyticsService {
  return new AnalyticsService(new PostgresAnalyticsCache(client.sql), (filters, period) => usageAnalytics(client.db, filters, period), (filters,page) => parameterPerformance(client.db, filters,page), (filters) => parameterUsage(client.db, filters));
}
