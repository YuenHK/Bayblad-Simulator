...
export function RhythmLaunch({ schedule, onCommand, reducedMotion = false, clockReady = true, clockSamples = 3 }: Readonly<{ schedule: LaunchScheduleView; onCommand: (command: CommandInput) => void; reducedMotion?: boolean; clockReady?: boolean; clockSamples?: number }>) {
  ...
  const launch = useCallback(() => {
    if (!clockReady || sentNonce.current === schedule.nonce) return false;
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      ...
      if (event.code === "Space" && clockReady && sentNonce.current !== schedule.nonce && !isInteractive) { event.preventDefault(); launch(); }
    };
    window.addEventListener("keydown", key);
    return () => window.removeEventListener("keydown", key);
  }, [launch]);
  const delta = schedule.serverTargetTimeMs - now;
  const liveText = !clockReady ? `正在同步時間 ${Math.min(clockSamples, 3)}/3` : sentNonce.current === schedule.nonce ? "已發射，等待判定" : delta <= 0 ? "發射" : delta <= 3_000 ? String(Math.ceil(delta / 1_000)) : "準備發射";
  return (
