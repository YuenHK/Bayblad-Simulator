...
describe("60 second cinematic boundaries",()=>{
  it("keeps the result hidden until the full minute",()=>{ expect(cinematicPhase(23999)).toBe("battle");expect(cinematicPhase(24000)).toBe("summon");expect(cinematicPhase(26999)).toBe("summon");expect(cinematicPhase(27000)).toBe("strike");expect(cinematicPhase(29999)).toBe("strike");expect(cinematicPhase(30000)).toBe("result"); });
  it("clamps the final approach",()=>{expect(cinematicProgress(24000)).toBe(0);expect(cinematicProgress(28500)).toBe(.5);expect(cinematicProgress(90000)).toBe(1);});
...
describe("geometry aware contact effects",()=>{
  ...
  it("interpolates buffered future frames only at their presentation time",()=>{
    const timed=(sequence:number,time:number):ArenaFrame=>({...frame(sequence,sequence*10,0),presentation:{startsAtMs:1000,durationMs:30000,elapsedMs:time,zodiacIndex:0,skillName:"test"}});
