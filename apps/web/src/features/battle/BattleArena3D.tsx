...
import { cinematicPhase, cinematicProgress, zodiacNumber, ZODIAC_NAMES, designRadiusMm } from "./zodiacScene";
import "./BattleArena3D.css";
import { arcadePose, impactState, shatterProgress } from "./arcadeMotion";
import { ElectricImpact, ShatteredTop } from "./ArcadeEffects";

export type BattleArena3DProps = Readonly<{ designs: readonly [TopDesign, TopDesign]; frames: readonly ArenaFrame[]; elapsedMs: number; displayDurationMs?: number; winner?: "player1" | "player2" | "draw" | undefined; zodiacIndex: number; skillName: string; reducedMotion?: boolean }>;
...
function AcrylicTop({ design }: { design: TopDesign }) {
  const geometries = useMemo(() => design.layers.map(layer => new ExtrudeGeometry(makeAcrylicShapes(layer, design), { depth: MATERIALS.layerThicknessMm, bevelEnabled: false, curveSegments: 16 })), [design]);
...
function Scene(props: BattleArena3DProps) {
  ...
  const impact = impactState(props.elapsedMs);
  useFrame(({ camera }) => {
    ...
    [top1, top2].forEach((ref, i) => {
      ...
      const pose=arcadePose(Math.min(props.elapsedMs,24000),radii)[i]!;
      let x=pose.x*.05, z=pose.y*.05;
      const progress = cinematicProgress(props.elapsedMs);
      if (phase === "strike" || phase === "result") {
        const approach = Math.min(1, Math.max(0, (progress-.25)/.45));
        x = x*(1-approach)+(i === 0 ? -1 : 1)*(radii[i]??25)*.05*approach; z *= 1-approach;
      ...
      object.visible=shatterProgress(props.elapsedMs,props.winner,side)===0;
      object.rotation.y = props.reducedMotion ? 0 : props.elapsedMs*.045*(i===0?1:-1);
    });
    if (beast.current) {
  ...
  return <>
    ...
    {phase === "battle" && <ElectricImpact ageMs={impact.ageMs} reducedMotion={props.reducedMotion??false}/>}
    {phase === "strike" && <ElectricImpact ageMs={props.elapsedMs-29100} reducedMotion={props.reducedMotion??false}/>}
    {(["player1","player2"] as const).map((side,i)=><ShatteredTop key={side} design={props.designs[i]!} progress={shatterProgress(props.elapsedMs,props.winner,side)} x={(i===0?-1:1)*(radii[i]??25)*.05} z={0} reducedMotion={props.reducedMotion??false}/>)}
    {!props.reducedMotion && phase === "battle" && Array.from({length:6},(_,i)=>arcadePose(Math.max(0,props.elapsedMs-(6-i)*45),radii).map((pose,j)=><mesh key={`${i}-${j}`} position={[pose.x*.05,.025,pose.y*.05]} rotation={[-Math.PI/2,0,0]}><ringGeometry args={[.45,.6,24]}/><meshBasicMaterial color={j === 0 ? "#35dfff" : "#ff5985"} transparent opacity={(i+1)*.065} depthWrite={false}/></mesh>))}
...
export function BattleArena3D(props: BattleArena3DProps) {
  ...
  return <section className="cinema-arena" data-testid="battle-arena-3d" data-phase={phase} data-shattered={props.winner && props.winner!=="draw" && props.elapsedMs>29100 ? (props.winner==="player1"?"player2":"player1") : "none"} aria-label="生肖神獸 3D 對戰場">
    <div className="cinema-hud"><span>生肖競技場 · 3D</span><span>{Math.max(0,Math.ceil((props.displayDurationMs??30000)*(1-props.elapsedMs/30000)/1000))} 秒</span></div>
