...
export function LobbyPage({ rooms, onCommand, disabled = false }: Readonly<{ rooms: readonly LobbyRoom[]; onCommand: (event: CommandInput) => void; disabled?: boolean }>) {
  const [roomName, setRoomName] = useState("我的陀螺房");
  const [code, setCode] = useState("");
  const [role, setRole] = useState<"player" | "spectator">("player");
  const [page, setPage] = useState(0);
  const pageSize = 50, pageCount = Math.max(1, Math.ceil(rooms.length / pageSize)), safePage = Math.min(page, pageCount - 1);
  const visibleRooms = rooms.slice(safePage * pageSize, (safePage + 1) * pageSize);
