...
export function RoomPage({ snapshot, battle, design, designId, onUseDesign, onCommand, onLeave, reducedMotion = false }: Readonly<{
  snapshot: RoomSnapshotEvent; battle: RoomBattleView; design: TopDesign; designId?: string | null;
  onUseDesign?: () => void | Promise<void>; onCommand: (event: CommandInput) => void; onLeave: () => void; reducedMotion?: boolean;
}>) {
  ...
  return <main className="app-shell"><header className="room-heading"><div><p className="eyebrow">房間碼 {snapshot.code}</p><h1>{snapshot.name}</h1></div><button type="button" onClick={onLeave} disabled={active && snapshot.viewer.role !== "spectator"}>{active && snapshot.viewer.role !== "spectator" ? "對戰完成後可離房" : "離開房間"}</button></header>
    <section className="seat-grid" aria-label="對戰玩家區">{([snapshot.player1, snapshot.player2] as const).map((seat, index) => <article className={`seat-card ${seat?.ready ? "is-ready" : ""}`} key={index}><span className="seat-number">玩家{index === 0 ? "一" : "二"}</span><h2>{seat?.displayName ?? "等待玩家"}{seat?.participantId === snapshot.ownerParticipantId ? <span title="房主" aria-label="房主"> ♛</span> : null}</h2><p>{seat ? (seat.ready ? "已準備·設計已鎖定" : seat.designId ? "已選設計，未準備" : "未準備") : "可自由補上空位"}</p>{snapshot.viewer.role === "spectator" && !active && seat === null ? <button onClick={() => onCommand({ type: "room.move", roomId: snapshot.roomId, target: index === 0 ? "player1" : "player2" })}>坐上此位</button> : null}</article>)}</section>
    <section className="room-controls panel" aria-label="房間操作">{snapshot.viewer.role !== "spectator" && !active ? <><button className="primary-button" onClick={() => void onUseDesign?.()}>{designId ? "以已選設計準備" : "上載當前設計並準備"}</button><button onClick={() => onCommand({ type: "room.move", roomId: snapshot.roomId, target: "spectator" })}>轉到觀賽區</button></> : null}{snapshot.viewer.isOwner && !active ? <button className="danger-button" onClick={() => onCommand({ type: "room.close", roomId: snapshot.roomId })}>關閉房間</button> : null}</section>
    {canLaunch ? <RhythmLaunch schedule={battle.schedule!} onCommand={onCommand} /> : null}
