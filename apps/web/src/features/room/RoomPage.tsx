...
export function RoomPage({ snapshot, battle, design, designId, onUseDesign, onCommand, onLeave, reducedMotion = false, departurePending = false }: Readonly<{
  ...
  return <main className="app-shell"><header className="room-heading"><div><p className="eyebrow">房間碼 {snapshot.code}</p><h1>{snapshot.name}</h1></div><button type="button" onClick={onLeave} disabled={departurePending || (active && snapshot.viewer.role !== "spectator")}>{departurePending ? "正在離房……" : active && snapshot.viewer.role !== "spectator" ? "對戰完成後可離房" : "離開房間"}</button></header>
