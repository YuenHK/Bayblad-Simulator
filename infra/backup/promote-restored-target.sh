...
acl_after=$(PGSERVICE=$PROMOTE_PGSERVICE psql -X -v ON_ERROR_STOP=1 -v role="$PROMOTE_APP_ROLE" -AtF '|' -f - <<<"select has_database_privilege('public',current_database(),'connect'),has_database_privilege(:'role',current_database(),'connect')");[[ $acl_after == 'f|f' ]]||die "ACL reopened after isolation"
