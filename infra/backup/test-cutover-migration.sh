#!/usr/bin/env bash
set -euo pipefail
: "${TEST_DATABASE_URL:?}";base=${TEST_DATABASE_URL%/*};db=steam_top_cutover_upgrade_$$;bad_db=steam_top_cutover_bad_$$;createdb --maintenance-db="$TEST_DATABASE_URL" "$db";createdb --maintenance-db="$TEST_DATABASE_URL" "$bad_db";cleanup(){ dropdb --if-exists --force --maintenance-db="$TEST_DATABASE_URL" "$db" >/dev/null 2>&1||true;dropdb --if-exists --force --maintenance-db="$TEST_DATABASE_URL" "$bad_db" >/dev/null 2>&1||true;};trap cleanup EXIT;url="$base/$db";apply_migration(){ psql "$url" -v ON_ERROR_STOP=1 -f drizzle/0001_cutover_state_machine.sql >/dev/null;}
