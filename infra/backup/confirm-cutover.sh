...
PGSERVICE=$PROMOTE_PGSERVICE psql -X -v ON_ERROR_STOP=1 -v nonce="$nonce" -v role="$role" -v target="$target" -v rows="$rows" -v smoke="$smoke_json" -v smoke_b64="$smoke_b64" -v smoke_sha="$smoke_sha" -v final="$final_json" -v final_sha="$final_sha" -v final_b64="$final_b64" -v preflight_sha="$preflight_sha" <<'SQL'
...
if [[ -f $final ]];then cmp -s "$expected" "$final"||die "final receipt conflict";rm -f "$expected";else mv "$expected" "$final";fi;chmod 400 "$final"
if [[ $existing_state == verified ]];then
  ...
  node -e 'require("fs").writeFileSync(process.argv[2],Buffer.from(process.argv[1],"base64"),{mode:0o400})' "$existing_signature_b64" "$final.sig"
else
  if [[ ! -f $final.sig ]];then ssh-keygen -Y sign -q -f "$CUTOVER_SIGNING_KEY" -n steam-top-public-cutover-smoke "$final";chmod 400 "$final.sig";fi
fi
ssh-keygen -Y verify -q -f "$CUTOVER_ALLOWED_SIGNERS_FILE" -I "$CUTOVER_SIGNER_ID" -n steam-top-public-cutover-smoke -s "$final.sig" <"$final"||die "final post-sign verify"
...
PGSERVICE=$PROMOTE_PGSERVICE psql -X -v ON_ERROR_STOP=1 -v nonce="$nonce" -v final_sha="$final_sha" -v final_b64="$final_b64" -v signature_b64="$signature_b64" -v signer="$CUTOVER_SIGNER_ID" -v role="$role" -v target="$target" -v rows="$rows" <<'SQL'
