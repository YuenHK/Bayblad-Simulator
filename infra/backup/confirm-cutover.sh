...
existing=$(PGSERVICE=$PROMOTE_PGSERVICE psql -X -v ON_ERROR_STOP=1 -AtF '|' -v nonce="$nonce" -f - <<<"select state,coalesce(final_receipt_sha256,''),coalesce(final_receipt_payload_b64,''),coalesce(final_receipt_signature_b64,''),coalesce(final_receipt_signer_id,'') from restore_control.finalize_outbox where nonce=:'nonce'")
