...
for name in PGSERVICE PGSERVICEFILE PGPASSFILE BACKUP_DIR AGE_RECIPIENT DELETION_LEDGER_FILE DELETION_LEDGER_CLI BACKUP_SIGNING_KEY BACKUP_SIGNER_ID BACKUP_ALLOWED_SIGNERS_FILE;do [[ -n ${!name:-} ]]||die "$name is required";done
...
[[ -f $DELETION_LEDGER_CLI && ! -L $DELETION_LEDGER_CLI && -O $DELETION_LEDGER_CLI ]]||die "ledger CLI is unsafe"
