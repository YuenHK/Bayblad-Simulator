...
deployment_root=$(CDPATH='' cd -- "$script_dir/../.."&&pwd -P);[[ $(id -u) -ne 0 ]]||die "backup runtime must be non-root"
stat_owner_mode(){ if stat -c '%u %a' "$1" >/dev/null 2>&1;then stat -c '%u %a' "$1";else stat -f '%u %Lp' "$1";fi;}
...
# BACKUP_DIR is a required environment variable checked above.
...
# PostgreSQL dollar quoting must remain literal shell text.
