...
if command -v shellcheck >/dev/null 2>&1; then
  shellcheck "$script_dir/backup.sh" "$script_dir/restore.sh" "$script_dir/host-trust-guard.sh" "$script_dir/verify-backup-set.sh" "$script_dir/verify-retention-set.sh" "$script_dir/enforce-retention.sh" "$script_dir/scrub-backups.sh" "$script_dir/verify-rollback-preflight.sh" "$script_dir/promote-restored-target.sh" "$0"
...
plain_dump="$test_root/plain.dump"; broken_dump="$test_root/broken.dump"; broken_backup="$test_root/backups/steam-top-20990101T000000Z-999999.backup";cp -R "$backup_file" "$broken_backup";rm -f "$broken_backup/VERIFIED.sig"
