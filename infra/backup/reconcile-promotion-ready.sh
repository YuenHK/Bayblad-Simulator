...
row=$(PGSERVICE="$PROMOTE_PGSERVICE" psql -X -v ON_ERROR_STOP=1 -v nonce="$nonce" -AtF '|' -f - <<<"select system_identifier,database_name,app_role,restore_target_id,ledger_rows,nonce from restore_control.promotion_outbox where nonce=:'nonce' and state='committed'");IFS='|' read -r system_id database role target rows bound_nonce <<EOF
$row
EOF
