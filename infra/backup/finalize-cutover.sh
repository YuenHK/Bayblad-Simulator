...
values=$(node -e 'const fs=require("fs"),crypto=require("crypto"),r=JSON.parse(fs.readFileSync(process.argv[1])),c=JSON.parse(fs.readFileSync(process.argv[2])),d=crypto.createHash("sha256").update(fs.readFileSync(process.argv[1])).digest("hex");if(r.schemaVersion!==2||c.schemaVersion!==1||c.readySha256!==d||c.publicSmoke!=="passed"||c.systemIdentifier!==r.systemIdentifier||c.database!==r.database||c.appRole!==r.appRole||c.restoreTargetId!==r.restoreTargetId||c.ledgerRows!==r.ledgerRows)process.exit(1);console.log([r.systemIdentifier,r.database,r.appRole,r.restoreTargetId,r.ledgerRows,c.nonce].join("|"))' "$ready" "$cutover")||die "bound cutover receipt"
