...
external_ledger_metadata=$(node "$DELETION_LEDGER_CLI" validate "$DELETION_LEDGER_FILE")
[[ $actual == "$sha256" && $external_ledger_metadata == *\"lines\":$deletion_ledger_lines* && $external_ledger_metadata == *\"sha256\":\"$deletion_ledger_sha256\"* && $ledger_actual == "$deletion_ledger_sha256" ]]||die "checksum or deletion ledger mismatch/older backup"
...
kill "$guard_pid" >/dev/null 2>&1||true;wait "$guard_pid";guard_pid=""
