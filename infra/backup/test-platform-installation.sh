#!/usr/bin/env bash
set -euo pipefail
: "${TEST_DATABASE_URL:?}";root=$(CDPATH= cd -- "$(dirname "$0")/../.."&&pwd -P);db=$(node -e 'console.log(new URL(process.argv[1]).pathname.slice(1))' "$TEST_DATABASE_URL");maintenance=${TEST_DATABASE_URL%/*}/postgres;service=$(mktemp);trap 'rm -f "$service";dropdb --if-exists --force --maintenance-db="$maintenance" "$db" >/dev/null 2>&1||true;createdb --maintenance-db="$maintenance" "$db" >/dev/null 2>&1||true' EXIT;dropdb --if-exists --force --maintenance-db="$maintenance" "$db";createdb --maintenance-db="$maintenance" "$db";url=$TEST_DATABASE_URL;for migration in "$root"/drizzle/000{0,1,2,3,4}_*.sql;do psql "$url" -v ON_ERROR_STOP=1 -f "$migration" >/dev/null;done;psql "$url" -v ON_ERROR_STOP=1 -c "create table app_schema_migrations(id text primary key,sha256 char(64) not null,applied_at timestamptz not null default now());insert into app_schema_migrations(id,sha256) values ('0000_steam_top_pre_first_deploy','51757c37f19429c162cbd2de8f04682e5e8dad0a78723fb142332e5f11d7721f'),('0001_cutover_state_machine','ca26cdef9195ae550a0fd4eb4db66fe02c2915e646a71e51182b4b4cf8a40571'),('0002_platform_installation','cb3dc38371bfaa56d14feb2f286be8dbeafc3dfe206dd0941d862973d7b60c62'),('0003_postgresql_catalog_array_compatibility','cec65962fe473861a97502548058a453a3a6ceda969e8b1fd414abede48ebc27'),('0004_scoped_readiness_acl','d1d75158a9fc771e412ee524b6a0d344bc0925e87199ad1e1f97db2cd8989aa6')" >/dev/null
psql "$url" -v ON_ERROR_STOP=1 -c "update public.app_schema_migrations set sha256='55a06f26947827ec40a29068fb17cd91a02b2085acaca15a1b2bb95f63c5aefb' where id='0002_platform_installation'" >/dev/null
...
[[ $(psql "$url" -Atqc "select count(*) from pg_attribute a join pg_class c on c.oid=a.attrelid join pg_namespace n on n.oid=c.relnamespace where n.nspname='public' and c.relname='app_schema_migrations' and a.attname='id' and a.attacl is not null") == 0 ]]
