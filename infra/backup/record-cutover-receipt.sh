...
const fs=require("fs"),state=JSON.parse(fs.readFileSync(process.argv[2])),host=JSON.parse(fs.readFileSync(process.argv[3])),ready=JSON.parse(fs.readFileSync(process.argv[6]));if(state.schemaVersion!==3||host.schemaVersion!==3||state.manifestSha256!==process.argv[4]||host.manifestDigest!==process.argv[4]||host.publicOrigin!==process.argv[5]||host.database?.restoreTargetId!==ready.restoreTargetId||host.database?.systemIdentifier!==ready.systemIdentifier)process.exit(1);
NODE
