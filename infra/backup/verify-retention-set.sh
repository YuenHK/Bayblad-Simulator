...
if command -v sha256sum >/dev/null 2>&1;then digest=$(sha256sum "$set_path/manifest"|awk '{print $1}');checksum_digest=$(sha256sum "$set_path/checksum.sha256"|awk '{print $1}');else digest=$(shasum -a 256 "$set_path/manifest"|awk '{print $1}');checksum_digest=$(shasum -a 256 "$set_path/checksum.sha256"|awk '{print $1}');fi
[[ $(<"$set_path/VERIFIED") == $'manifest_sha256='"$digest"$'\nchecksum_file_sha256='"$checksum_digest" ]]
