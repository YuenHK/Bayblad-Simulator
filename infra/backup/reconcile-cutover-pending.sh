...
while IFS='|' read -r nonce role target system database ledger ready_sha preflight_sha generation;do
  ...
  result=$(PGSERVICE=$PROMOTE_PGSERVICE psql -X -v ON_ERROR_STOP=1 -v nonce="$nonce" -v role="$role" -v target="$target" -v system="$system" -v database="$database" -v ledger="$ledger" -v generation="$generation" -At <<'SQL'
begin;select pg_advisory_xact_lock(1937002751);select case when current_database()=:'database' and (select system_identifier::text from pg_control_system())=:'system' and exists(select 1 from restore_control.deployment_environment where singleton and environment='production' and not restore_allowed and restore_target_id=:'target'::uuid) and (select count(*) from deletion_audit)=:'ledger'::bigint then 1 else 1/0 end;select format('revoke connect on database %I from %I',current_database(),:'role') \gexec
