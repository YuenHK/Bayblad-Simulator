...
[[ $(id -u) -eq 0 && $# -eq 6 && $1 =~ ^[a-f0-9]{64}$ && $2 =~ ^[a-f0-9]{64}$ ]]||exit 2
activation_nonce=$1;legacy_nonce=$2;receipt=$3;signature=$4;ready=$5;ready_sha=$6;tmp=$(mktemp -d);trap 'rm -rf "$tmp"' EXIT
...
/opt/steam-top-bootstrap/resolve-production-state.sh "$activation_nonce" >"$tmp/frame";runtime=$(awk '/^STATE-BEGIN /{print $5;exit}' "$tmp/frame");[[ $runtime == /opt/steam-top/releases/* && -d $runtime && ! -L $runtime ]]||exit 1;runtime_sha=$(basename "$runtime");RUNTIME_INSTALL_MANIFEST_SHA256="$runtime_sha" "$runtime/scripts/verify-runtime-install.sh" "$runtime"
env CANONICAL_STATE_RESOLVED=true RUNTIME_INSTALL_MANIFEST_SHA256="$runtime_sha" PROMOTE_PGSERVICE="${config[3]}" PGSERVICEFILE="${config[4]}" PGPASSFILE="${config[5]}" "$runtime/infra/backup/import-legacy-cutover.sh" "$legacy_nonce" "$receipt" "$signature" "$ready" "$ready_sha" "${config[1]}" "${config[2]}"
