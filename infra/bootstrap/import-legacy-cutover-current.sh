...
[[ $(id -u) -eq 0 && $# -eq 8 && $1 =~ ^[a-f0-9]{64}$ && $2 =~ ^[a-f0-9]{64}$ ]]||exit 2
activation_nonce=$1;legacy_nonce=$2;receipt=$3;signature=$4;ready=$5;ready_sha=$6;preflight=$7;preflight_signature=$8;tmp=$(mktemp -d);trap 'rm -rf "$tmp"' EXIT
...
env CANONICAL_STATE_RESOLVED=true RUNTIME_INSTALL_MANIFEST_SHA256="$(basename "$runtime")" PROMOTE_PGSERVICE="${config[3]}" PGSERVICEFILE="${config[4]}" PGPASSFILE="${config[5]}" "$runtime/infra/backup/import-legacy-cutover.sh" "$legacy_nonce" "$receipt" "$signature" "$ready" "$ready_sha" "$preflight" "$preflight_signature" "${config[1]}" "${config[2]}"
