...
env CANONICAL_STATE_RESOLVED=true RUNTIME_INSTALL_MANIFEST_SHA256="$(basename "$runtime")" PROMOTE_PGSERVICE="${config[3]}" PGSERVICEFILE="${config[4]}" PGPASSFILE="${config[5]}" "$runtime/infra/backup/import-legacy-cutover.sh" "$legacy_nonce" "$receipt" "$signature" "$ready" "$ready_sha" "${config[1]}" "${config[2]}"
