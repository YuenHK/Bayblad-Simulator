...
if [[ ! -e /opt/steam-top/current ]];then
  ...
  if [[ $first_phase == db-claimed ]];then /opt/steam-top-bootstrap/advance-first-deploy-state.sh consumed "$first_nonce";PGSERVICE=${values[0]} PGSERVICEFILE=${values[1]} PGPASSFILE=${values[2]} psql -X -v ON_ERROR_STOP=1 -q -v nonce="$first_nonce" -c "update restore_control.platform_installation set generation=generation+1 where singleton and authorization_nonce=:'nonce'" >/dev/null;fi
fi
