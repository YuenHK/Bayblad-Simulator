...
if [[ ! -e /opt/steam-top/current ]];then
  ...
  if [[ $first_phase == db-claimed ]];then changed=$(PGSERVICE=${values[0]} PGSERVICEFILE=${values[1]} PGPASSFILE=${values[2]} psql -X -v ON_ERROR_STOP=1 -qAt -v nonce="$first_nonce" -v host="$first_host" -v digest="$first_digest" -c "update restore_control.platform_installation set generation=2 where singleton and authorization_nonce=:'nonce' and host_id=:'host' and bootstrap_digest=:'digest' and generation=1 returning 1");if [[ -z $changed ]];then current_generation=$(PGSERVICE=${values[0]} PGSERVICEFILE=${values[1]} PGPASSFILE=${values[2]} psql -X -qAt -v nonce="$first_nonce" -c "select generation from restore_control.platform_installation where authorization_nonce=:'nonce'");[[ $current_generation == 2 ]]||die "database installation consume CAS failed";fi;/opt/steam-top-bootstrap/advance-first-deploy-state.sh consumed "$first_nonce";first_phase=consumed;fi
