...
ceremony_id=$(sha /opt/steam-top-bootstrap/bootstrap-files.sha256);ceremony_root=/opt/steam-top-policy-ceremony;ceremony_path=$ceremony_root/$ceremony_id;install -d -o root -g root -m 0555 "$ceremony_root"
ceremony_stage=$(mktemp -d "$ceremony_root/.stage.XXXXXX");for name in verify-and-create-production-policy-entry.sh verify-production-policy-anchor.mjs create-production-policy-ledger-entry.mjs verify-attestation-identity.mjs;do mode=0444;[[ $name == *.sh ]]&&mode=0555;install -o root -g root -m "$mode" "/opt/steam-top-bootstrap/$name" "$ceremony_stage/$name";done;chmod 0555 "$ceremony_stage";sync -f "$ceremony_stage"
...
ceremony_manifest="$tmp/policy-ceremony-manifest.json";node - "$ceremony_manifest" "$ceremony_path" "$(sha "$ceremony_path/verify-and-create-production-policy-entry.sh")" "$(sha "$ceremony_path/verify-production-policy-anchor.mjs")" "$(sha "$ceremony_path/create-production-policy-ledger-entry.mjs")" "$(sha "$ceremony_path/verify-attestation-identity.mjs")" <<'NODE'
const fs=require("fs"),[out,generationPath,verify,anchor,create,attestation]=process.argv.slice(2);fs.writeFileSync(out,JSON.stringify({schemaVersion:1,purpose:"steam-top-production-policy-ceremony",generationPath,files:{"verify-and-create-production-policy-entry.sh":verify,"verify-production-policy-anchor.mjs":anchor,"create-production-policy-ledger-entry.mjs":create,"verify-attestation-identity.mjs":attestation}})+"\n",{mode:0o400})
NODE
