...
for name in verify-bootstrap.sh verify-reaper-health.sh advance-first-deploy-state.sh read-first-deploy-state.sh key-custody-guard.sh verify-attestation-identity.mjs inspect-generation-chain.mjs prepare-release.sh deploy-release.sh fetch-receipt.sh fetch-deployment-evidence.sh activate-production-state.sh resolve-production-state.sh publish-generation.sh record-cutover-current.sh finalize-current.sh confirm-cutover-current.sh abort-cutover-current.sh reconcile-cutover-pending.sh import-legacy-cutover-current.sh;do [[ -f $source_dir/$name && ! -L $source_dir/$name ]]||exit 1;mode=0444;[[ $name == *.sh ]]&&mode=0555;install -m "$mode" "$source_dir/$name" "$tmp/$name";done
