...
for name in invoke-production-policy-signer.py sign-production-policy-entry.py;do mode=0444;[[ $name == invoke-* ]]&&mode=0555;printf '%s %s %s\n' "$(sha "$tmp/$name")" "$mode" "$name" >>"$tmp/bootstrap-files.sha256";done
