...
"$runtime/scripts/verify-runtime-install.sh" "$runtime"||die "runtime seal"
env CANONICAL_STATE_RESOLVED=true CANONICAL_DEPLOYMENT_PURPOSE="$purpose" PROTECTED_DEPLOYMENT_STATE_FILE="$tmp/state" PROTECTED_DEPLOYMENT_STATE_SIGNATURE="$tmp/state.sig" ACTIVATION_RECEIPT_FILE="$tmp/activation" ACTIVATION_RECEIPT_SIGNATURE="$tmp/activation.sig" "$runtime/infra/backup/confirm-cutover.sh" "$@";/opt/steam-top-bootstrap/resolve-production-state.sh "$nonce" >"$tmp/after";cmp -s "$tmp/frame" "$tmp/after"
