...
read -r outbox key < <(node - "$config" <<'NODE'
const x=require(process.argv[2]);if(!x.hostReceiptOutbox?.startsWith("/")||!x.evidenceSigningKey?.startsWith("/"))process.exit(1);console.log(x.hostReceiptOutbox,x.evidenceSigningKey)
NODE
)
...
ssh-keygen -Y verify -q -f <(printf 'evidence %s\n' "$(ssh-keygen -y -f "$key")") -I evidence -n steam-top-deployment-authorization-evidence -s "$signature" <"$payload"||exit 1
