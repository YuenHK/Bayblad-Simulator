...
for file in "$config" "$manifest";do [[ -f $file && ! -L $file ]]||die "sealed file"||exit 1;read -r owner mode < <(stat -c '%u %a' "$file");[[ $owner == 0 && $mode == 400 ]]||die "sealed root-owned exact mode"||exit 1;done
