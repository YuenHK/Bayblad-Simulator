...
NODE
ssh-keygen -Y sign -q -f "$key" -n steam-top-first-deploy-state "$tmp/payload.json";chmod 0400 "$tmp/payload.json.sig";id=$(sha256sum "$tmp/payload.json"|awk '{print $1}');chmod 0500 "$tmp";sync -f "$tmp";mv "$tmp" "$root/generations/$id";sync -f "$root/generations";ln -s "generations/$id" "$root/.current.$$.tmp";mv -Tf "$root/.current.$$.tmp" "$root/current";sync -f "$root"
