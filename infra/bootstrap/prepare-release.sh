...
const x=require(process.argv[2]);for(const k of ["repository","workflowIdentity","workflowRef","workflowSha"])if(typeof x[k]!=="string")process.exit(1);console.log(x.repository);console.log(x.workflowIdentity);console.log(x.workflowRef);console.log(x.workflowSha);
NODE
);repository=${trust[0]};workflow_identity=${trust[1]};workflow_ref=${trust[2]};workflow_sha=${trust[3]}
...
node /opt/steam-top-bootstrap/verify-attestation-identity.mjs "$snapshot/authorization-attestation.json" "$config"||die "protected workflow ref/digest"
values=$(node - "$snapshot/deployment-authorization.json" "$snapshot/pending.json" "$snapshot/release-manifest.json" "$snapshot/runtime-files.sha256" "${snapshot}/APPROVED-RELEASE.json" "$repository" <<'NODE'
const fs=require("fs"),crypto=require("crypto"),a=require(process.argv[2]),p=require(process.argv[3]),m=require(process.argv[4]),runtime=fs.readFileSync(process.argv[5]),approvedPath=process.argv[6],repo=process.argv[7],digest=x=>crypto.createHash("sha256").update(x).digest("hex"),approved=fs.existsSync(approvedPath)?require(approvedPath):null;if(a.schemaVersion!==1||a.purpose!=="production-deployment-authorization"||a.repository!==repo||p.payload?.purpose!=="production"||a.commit!==m.commit||a.commit!==p.ref||a.manifestSha256!==digest(fs.readFileSync(process.argv[4]))||a.runtimeManifestSha256!==digest(runtime)||a.pendingSha256!==digest(fs.readFileSync(process.argv[3]))||a.deploymentId!==String(p.payload.deploymentId??a.deploymentId)||a.nonce!==p.payload.nonce||a.expectedPreviousState!==p.payload.expectedPreviousState)process.exit(1);if(a.signerKind==="normal"&&(!approved||approved.purpose!=="approved-release"||approved.commit!==a.commit||approved.manifestSha256!==a.manifestSha256||a.approvedMarkerSha256!==digest(fs.readFileSync(approvedPath))))process.exit(1);console.log([a.runtimeManifestSha256,a.commit,a.signerWorkflow].join("|"));
NODE
...
RUNTIME_INSTALL_MANIFEST_SHA256=$runtime_sha DEPLOYMENT_AUTHORIZATION_PURPOSE=production "$release_root/scripts/prepare-deployment-authorization.sh" "$artifact" "$pending" "$repository" "$(node -p 'require(process.argv[1]).deploymentId' "$bundle")" "$token" "$state_dir"
