...
const fs=require("fs"),crypto=require("crypto"),a=require(process.argv[2]),p=require(process.argv[3]),m=require(process.argv[4]),runtime=fs.readFileSync(process.argv[5]),approvedPath=process.argv[6],repo=process.argv[7],purpose=process.argv[8],digest=x=>crypto.createHash("sha256").update(x).digest("hex"),approved=fs.existsSync(approvedPath)?require(approvedPath):null;if(a.schemaVersion!==1||a.purpose!==`${purpose}-deployment-authorization`||a.deploymentPurpose!==purpose||a.repository!==repo||p.payload?.purpose!==purpose||a.commit!==m.commit||a.commit!==p.ref||a.candidateRunId!==p.payload.runId||a.candidateArtifact!==p.payload.artifact||a.candidateRef!==p.payload.sourceRef||a.candidateHeadSha!==p.payload.sourceHeadSha||a.candidateEvent!==p.payload.sourceEvent||a.manifestSha256!==digest(fs.readFileSync(process.argv[4]))||a.runtimeManifestSha256!==digest(runtime)||a.pendingSha256!==digest(fs.readFileSync(process.argv[3]))||a.deploymentId!==String(p.payload.deploymentId??a.deploymentId)||a.nonce!==p.payload.nonce||a.expectedPreviousState!==p.payload.expectedPreviousState)process.exit(1);if(a.signerKind==="normal"&&(!approved||approved.purpose!=="approved-release"||approved.commit!==a.commit||approved.manifestSha256!==a.manifestSha256||a.approvedMarkerSha256!==digest(fs.readFileSync(approvedPath))))process.exit(1);console.log([a.runtimeManifestSha256,a.commit,a.signerWorkflow].join("|"));
NODE
