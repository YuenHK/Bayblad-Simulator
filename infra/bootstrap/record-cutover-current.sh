...
exec env CANONICAL_STATE_RESOLVED=true PROTECTED_DEPLOYMENT_STATE_FILE="$tmp/state" PROTECTED_DEPLOYMENT_STATE_SIGNATURE="$tmp/state.sig" ACTIVATION_RECEIPT_FILE="$tmp/activation" ACTIVATION_RECEIPT_SIGNATURE="$tmp/activation.sig" /opt/steam-top/current/infra/backup/record-cutover-receipt.sh "$ready" "$receipt"
