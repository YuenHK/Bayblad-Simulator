...
if os.geteuid()!=0 and len(sys.argv)>2 and sys.argv[1]=="--test-manifest":MANIFEST=sys.argv[2];OWNER=os.getuid();TEST_ONLY=True;del sys.argv[1:3]
...
os.close(mf);os.close(signer);os.close(runtime)
if TEST_ONLY:print("verified");raise SystemExit(0)
os.execve(value["pythonPath"],[value["pythonPath"],"-I","-E","-S",value["signerPath"],*sys.argv[1:]],{"PATH":"/usr/bin:/bin","LANG":"C","LC_ALL":"C","PYTHONNOUSERSITE":"1"})
