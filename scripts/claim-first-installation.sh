...
root=$(CDPATH= cd -- "$script_dir/.."&&pwd -P);read -r protect_sha deletion_sha pristine_sha < <(node "$script_dir/derive-restore-function-hashes.mjs" "$root/drizzle/0000_steam_top_pre_first_deploy.sql" "$root/drizzle/0001_cutover_state_machine.sql" "$root/drizzle/0002_platform_installation.sql" "$root/drizzle/0003_postgresql_catalog_array_compatibility.sql" "$root/drizzle/0004_scoped_readiness_acl.sql");for value in "$protect_sha" "$deletion_sha" "$pristine_sha";do [[ $value =~ ^[a-f0-9]{64}$ ]]||die "canonical function hash";done
{ cat <<'SQL'
begin;select pg_advisory_xact_lock(1937002751);select set_config('steam_top.expected_platform_migration_sha','d1d75158a9fc771e412ee524b6a0d344bc0925e87199ad1e1f97db2cd8989aa6',true);select set_config('steam_top.expected_protect_function_sha',:'protect_sha',true),set_config('steam_top.expected_deletion_function_sha',:'deletion_sha',true),set_config('steam_top.expected_pristine_function_sha',:'pristine_sha',true);
SQL
cat "$catalog_sql"
cat <<'SQL'
