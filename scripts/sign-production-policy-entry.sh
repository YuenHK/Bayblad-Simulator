...
/usr/bin/node -e 'const fs=require("fs"),p=process.argv[1],t=fs.readFileSync(p,"utf8"),x=JSON.parse(t),y={schemaVersion:1,purpose:"production-policy-root-rotation",generation:x.generation,previousReceiptDigest:x.previousReceiptDigest,repositoryId:x.repositoryId,repositoryName:x.repositoryName,policyCommit:x.policyCommit,policyTreeOid:x.policyTreeOid,bundleSha256:x.bundleSha256,anchorSha256:x.anchorSha256,anchorGeneration:x.anchorGeneration,createdAt:x.createdAt,signerKeyId:x.signerKeyId};if(t!==JSON.stringify(y)+"\n")process.exit(1)' "$2"||{ printf 'entry is not canonical\n' >&2;exit 65; }
set -C
