...
-- Invoker-rights triggers call these read-only helpers while validating writes.
-- Keep every other public function unavailable to the application role.
GRANT EXECUTE ON FUNCTION public.steam_top_assert_battle_eligible_design_layers(uuid), public.steam_top_current_delete_is_audited(), public.digest(text,text) TO steam_top_app;
DO $assert$ BEGIN
IF EXISTS(SELECT 1 FROM pg_auth_members m JOIN pg_roles r ON r.oid=m.member JOIN pg_roles p ON p.oid=m.roleid WHERE r.rolname='steam_top_app' OR p.rolname='steam_top_app')
 ...
 OR EXISTS(SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND has_function_privilege('steam_top_app',p.oid,'EXECUTE') IS DISTINCT FROM (p.oid=ANY(ARRAY['public.steam_top_assert_battle_eligible_design_layers(uuid)'::regprocedure,'public.steam_top_current_delete_is_audited()'::regprocedure,'public.digest(text,text)'::regprocedure])))
