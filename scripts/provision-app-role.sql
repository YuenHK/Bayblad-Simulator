...
SELECT format('REVOKE CONNECT ON DATABASE %I FROM steam_top_app',datname) FROM pg_database WHERE datallowconn AND datname<>current_database() ORDER BY datname \gexec
...
IF EXISTS(SELECT 1 FROM pg_auth_members m JOIN pg_roles r ON r.oid=m.member JOIN pg_roles p ON p.oid=m.roleid WHERE r.rolname='steam_top_app' OR p.rolname='steam_top_app')
 ...
 OR EXISTS(SELECT 1 FROM pg_database d,aclexplode(d.datacl) a JOIN pg_roles r ON r.oid=a.grantee WHERE d.datallowconn AND d.datname<>current_database() AND r.rolname='steam_top_app' AND a.privilege_type='CONNECT')
