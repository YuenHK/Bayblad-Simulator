...
if [[ $probe_code == 200 ]];then grep -Fq "$nonce" "$tmp/probe";second_probe_code=$(curl --silent --show-error --resolve "$resolve_tls" -b "$tmp/cookies" -H "Origin: $origin" -o /dev/null -w '%{http_code}' "$origin/api/admin/deployment-probe/$nonce");[[ $second_probe_code == 404 ]]||{ echo "production smoke repeated probe HTTP $second_probe_code" >&2;exit 1;};elif [[ $probe_code != 404 || ${PRODUCTION_SMOKE_PROBE_ALREADY_OBSERVED:-false} != true ]];then echo "production smoke probe HTTP $probe_code" >&2;exit 1;fi
