...
for directory in "$previous" "$current";do [[ $directory == /* && -d $directory && ! -L $directory ]]||exit 1;"$script_dir/portable-sha256.sh" check "$directory" "$directory/SHA256SUMS";done
...
"$script_dir/portable-sha256.sh" manifest "$output" release-manifest.json >"$output/SHA256SUMS"
