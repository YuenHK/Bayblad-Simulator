...
const [readyPath,databaseUrl,origin,manifestDigest,nonce,output]=process.argv.slice(2),ready=JSON.parse(readFileSync(readyPath,"utf8")),url=new URL(databaseUrl);
if(!/^postgres(?:ql)?:$/u.test(url.protocol)||!/^https:\/\/[a-z0-9.-]+(?::[0-9]+)?$/u.test(origin)||!/^[a-f0-9]{64}$/u.test(manifestDigest)||!/^[a-f0-9]{64}$/u.test(nonce)||ready.schemaVersion!==1)throw new Error("cutover receipt input invalid");
