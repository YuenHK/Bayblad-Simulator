...
intent=$1;bundle=$2;anchor=$3;repository=$4;expected_run=$5;expected_attempt=$6;expected_bundle=$7;signer=$8;output=$9
...
const fs=require("fs"),intent=JSON.parse(fs.readFileSync(process.argv[2])),anchor=JSON.parse(process.argv[3]),repo=process.argv[4],run=process.argv[5],attempt=process.argv[6],bundle=process.argv[7],trust=process.argv[8],identity=anchor.rotationWorkflowRef.split("@")[0],ref=anchor.rotationWorkflowRef.slice(identity.length+1);
if(intent.repositoryName!==repo||anchor.rotationRepositoryName!==repo||intent.repositoryId!==anchor.rotationRepositoryId||String(intent.runId)!==run||String(intent.runAttempt)!==attempt||intent.bundleSha256!==bundle||intent.anchorSha256!==anchor.anchorSha256||intent.anchorGeneration!==anchor.anchorGeneration||intent.nextGeneration!==anchor.ledgerGeneration+1||intent.previousReceiptDigest!==anchor.ledgerReceiptDigest||intent.trustedWorkflowRef!==anchor.rotationWorkflowRef||intent.trustedWorkflowSha!==anchor.rotationWorkflowSha)process.exit(1);
