...
if [[ ${DEPLOYMENT_AUTHORIZATION_PURPOSE:-production} == production && $claim_phase == consumed ]];then
  PGSERVICE=$(node -p 'require("/etc/steam-top-bootstrap/trust.json").cutoverPgService') PGSERVICEFILE=$(node -p 'require("/etc/steam-top-bootstrap/trust.json").cutoverPgServiceFile') PGPASSFILE=$(node -p 'require("/etc/steam-top-bootstrap/trust.json").cutoverPgPassFile') psql -X -qAt -v host="$claim_host" -v digest="$claim_digest" -v nonce="$claim_nonce" -f - <<<"select case when count(*)=1 then 1 else 1/0 end from restore_control.platform_installation where host_id=:'host' and bootstrap_digest=:'digest' and authorization_nonce=:'nonce' and generation>=2" >/dev/null||die "established database authority mismatch"
fi
if [[ ${DEPLOYMENT_AUTHORIZATION_PURPOSE:-production} == production ]];then
