...
if(previous.schemaVersion!==1||current.schemaVersion!==1||previous.commit!==previousCommit||current.commit!==currentCommit||previous.images?.migration!==previous.images?.server||current.images?.migration!==current.images?.server||state.commit!==currentCommit||!/^\d+$/u.test(state.deploymentId)||!/^[a-f0-9]{64}$/u.test(state.manifestSha256))throw new Error("source identity invalid");
...
writeFileSync(output,JSON.stringify({schemaVersion:1,commit:previousCommit,images,rollbackSources:{applicationCommit:previousCommit,databaseCommit:currentCommit,databaseEnginePreserved:true,currentDeploymentId:state.deploymentId,currentDeploymentCreatedAt:state.deploymentCreatedAt,currentManifestSha256:state.manifestSha256}},null,2)+"\n",{flag:"wx",mode:0o600});
