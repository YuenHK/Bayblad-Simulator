...
[[ $runtime_root == /opt/steam-top && -f $manifest && ! -L $manifest && -f $receipt && ! -L $receipt ]]||runtime_die "canonical sealed files required"||exit 1
...
node - "$receipt" "$actual_manifest" <<'NODE'
const fs=require("fs"),r=JSON.parse(fs.readFileSync(process.argv[2]));
if(r.schemaVersion!==1||r.purpose!=="steam-top-runtime-install"||r.installRoot!=="/opt/steam-top"||r.runtimeManifestSha256!==process.argv[3]||!/^[a-f0-9]{40}$/.test(r.commit)||typeof r.sealedAt!=="string")process.exit(1);
NODE
