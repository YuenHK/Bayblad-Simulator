...
const signalHandlers=new Map([["SIGHUP",1],["SIGINT",2],["SIGTERM",15]].map(([signal,number])=>[signal,()=>{activeGitChild?.kill("SIGTERM");cleanup();process.exit(128+number)}]));
...
const git=(args,options={})=>new Promise((resolve,reject)=>{const child=execFile(GIT,args,{encoding:null,...options,env:{...baseEnvironment,...options.env}},(error,stdout)=>{if(activeGitChild===child)activeGitChild=null;error?reject(error):resolve(stdout)});activeGitChild=child;if(typeof process.send==="function")process.send({purpose:"steam-top-policy-git-child-running"})});
