...
gh attestation verify "$snapshot/runtime-files.sha256" --repo "$repository" --signer-workflow "$signer_workflow" >/dev/null||die "runtime manifest attestation"
runtime_manifest_sha=$("$root/scripts/portable-sha256.sh" digest "$snapshot/runtime-files.sha256");RUNTIME_INSTALL_MANIFEST_SHA256=$runtime_manifest_sha "$root/scripts/verify-runtime-install.sh" "$root"||die "runtime install trust"
