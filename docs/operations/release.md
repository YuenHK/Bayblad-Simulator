...
Deployment writer policy另由受保護 `production-release-policy-approval` environment 的 `PRODUCTION_POLICY_BUNDLE_SHA256` 外部固定。兩位管理員先審閱 default branch 上所有 authority workflows及 verifier，執行 `./scripts/print-production-policy-pin.sh`，核對輸出的 bundle digest，再按輸出的 `gh variable set` 命令輪換 environment variable；branch 合併與 environment variable 更新須分開審批並留下 audit record。`authorize-release.yml` 只接受 trigger-time workflow SHA 是目前 default branch 的祖先，並以該 protected checkout內的 absolute verifier檢查 candidate；live default branch可前進，不會令已觸發的合法release錯誤失效。缺少或不符外部digest會在任何 deployment writer執行前以 `protected production policy bundle digest mismatch` fail closed。候選同步修改workflow、verifier及內嵌writer hash亦不能改變此environment pin。

Installer 與 pretrusted tar validator 來源必須同時受 `INSTALLER_ALLOWED_HOSTS`、無 redirect HTTPS/TLS 1.3、各自的外部 `INSTALLER_SHA256`/`VALIDATOR_SHA256` 及 SSH signature identity/namespace 限制。Validator 必須以 root-owned `0444` 安裝於受信任不可寫目錄，installer 在開啟 archive 前再核對其 pinned digest，然後只呼叫這一份 validator，不可在 installer 內維護另一份 parser。輪換時要先由主機管理員審閱新 bytes/簽署，再同步更新受保護 variables，不可只改 URL。Hard gate 保留 `gh attestation verify --format json` 的 bootstrap 與 authorization 真實 evidence，並使用與 production bootstrap 相同的 exact subject/ref/workflow SHA verifier；schema 漂移必須 fail closed。

`install-bootstrap.sh` 是主機 image provisioning 前置條件，不允許從 candidate path 直接 sudo。Bootstrap archive 亦必須包含同一 canonical installer並列入已簽 `bootstrap-files.sha256`，讓 preflight、install receipt及已安裝 tree 核對 installer digest；archive 內副本絕不取代既有主機信任。主機管理員必須以校外已審核 `INSTALLER_SHA256` 及 `steam-top-bootstrap-installer` SSH signature 驗證 bytes，再安裝為 root-owned 0555 `/usr/local/sbin/install-steam-top-bootstrap`；sudoers 只可列出這個 canonical pinned path。更新 installer 時必須同時輪換外部 digest/signature，不可由 release artifact 自證。

外部輸入會先由 root 以 `O_NOFOLLOW` fd 單次複製 archive 及 signature 到 root-private `0444` snapshot，逐檔及目錄 fsync；之後 digest、signature、validator、extract 及 package-tree 驗證全部只讀取同一 snapshot，完成後刪除，避免 runner-owned path 置換。Pinned installer 再呼叫外置 pinned validator 檢查每個 tar header，只接受 manifest唯一列出的 canonical relative regular files、root uid/gid、精確 mode、受限 size/count及逐檔 digest；absolute／`..`、symlink、hardlink、device、FIFO、PAX、GNU long-name及 sparse metadata一律在 `/opt`、`/etc`、`/var`首次 mutation前拒絕。簽署壓縮檔上限為 8 MiB，串流解析另限制64個header、單檔2 MiB、總展開16 MiB及64倍壓縮比。
...
Confirm、abort 與 reaper 共用同一 PostgreSQL advisory lock；row 必須在同一 transaction 內 `FOR UPDATE` 並重新核對 state、lease generation、deadline、artifact digests、deletion ledger row count 及 canonical SHA-256，才可 grant、revoke 或轉換狀態。CI 以兩個實際程序排隊在同一 advisory-lock barrier，分別證明 confirm 先到時維持 `verified/CONNECT=true`，abort 先到時維持 `aborted/CONNECT=false`。Workflow 的 exact-nonce recovery 使用 `always()`；只有驗證 `aborted|CONNECT=false|sessions=0` 才容許刪除測試 volume，否則保存 volume 並上傳 incident evidence。
...
舊版 `committed` cutover migration 後只會成為 `legacy-committed`，絕不等同 APPROVED。管理員須保留歷史 one-stage schema v2 receipt、receipt 原簽署及 ready/sha；receipt 必須是 purpose `production`、namespace `steam-top-cutover` 的14個 exact keys，使用 `promotionNonce` 及字串 `publicSmoke: "passed"`。Canonical importer 會同時核對 live database、production marker 及 deletion ledger hash，並在同一 transaction 永久保存 receipt/signature、ready/sha 及推導後 ledger hash/app role；欠任一證據必須保持 legacy 狀態。

正式安裝 bootstrap 時 systemd 是強制 invariant：production 預設等同 `--install-systemd`。只有真正全新、從未存在 bootstrap install receipt、current、protected state 或已消耗標記的主機，provisioning 才可明確加上 `--initialize-first-deploy <64-hex nonce>`；安裝器會建立綁定 host/bootstrap digest、authorization nonce 及 install receipt digest 的簽署 append-only generation。升級絕不可帶此參數。部署依序只啟動 DB、執行 migration，再以 `verify-reaper-health.sh --post-migration-first-deploy` 核對 outbox table 存在且沒有 pending/smoke-observed rows，通過後才可啟動其餘服務；activation 後的 strict health 只會原子發布 consumed generation，禁止重建或回退。安裝器亦會驗證、複製及啟用 timer，立即執行 oneshot，並核對 timer active、service Result 及 ExecMainStatus。任一步失敗必須 disable timer 並回滾 unit。只有 purpose 為 `release-integration` 才可明確使用 `--no-systemd-for-integration`。Legacy receipt 亦不得直接呼叫 raw script，只可經 `/opt/steam-top-bootstrap/import-legacy-cutover-current.sh ACTIVATION_NONCE LEGACY_NONCE RECEIPT RECEIPT_SIG READY READY_SHA`；歷史 `0440` evidence 會先被複製成 root-private snapshot。Confirm 只由 canonical wrapper 輸出一行 JSON frame，receipt 固定在 root-owned `0700` incident state directory，caller 不可指定任意輸出路徑。
...
首次安裝的 authorization nonce 必須由 canonical `deploy-release.sh` 明確傳給 deploy core；任何缺失、損壞或非預期 first-deploy phase 都會拒絕部署。Install transaction journal、唯一 install receipt及 first-deploy generation 分別使用獨立 SSH signature namespace，並綁定 live host ID、bootstrap manifest、initial nonce及 receipt digest。重啟只可沿同一 previous-digest chain重播；多 receipt、孤立 generation、pointer rollback或 DB singleton不一致均須 fail closed。Fresh DB 的 migration seed（production/non-restore marker及未暫停 platform settings）是唯一容許的非空 baseline；其他 public durable table、restore outbox/audit或未知 schema drift均拒絕 first claim。

Production Compose 只將 PostgreSQL control port 綁到主機 loopback `127.0.0.1:15432`，供 root-owned、TLS `verify-full` 的 PGSERVICE 執行 claim、reaper 及 cutover；防火牆不得把此 port 轉送至 LAN。正式環境是**專用 PostgreSQL cluster**：除目前 production database、`postgres`、`template0`、`template1` 外不得存在任何 database（即使 `ALLOW_CONNECTIONS=false` 亦拒絕），owner、template flag及連線狀態亦須符合 canonical 初始化狀態。角色 provisioning 會在任何持久 mutation 前檢查此 invariant；shared cluster 一律 fail closed。成立後才撤銷 app/PUBLIC 對 `postgres`、`template0`、`template1` 的 CONNECT，並只對目前 database 授予 `steam_top_app` CONNECT。Migration及 first claim只用 DB owner control identity；pristine claim完成後才建立 `steam_top_app`（no-superuser/no-createdb/no-createrole/noinherit），server的 canonical `DATABASE_URL` 固定目前 production database，並以 Compose `!override` 只掛載獨立 app URL secret，owner secret僅供 migration/control。當前 database 內會撤銷角色繼承、schema/table/sequence/function/default privileges及 PUBLIC schema CREATE，再精確授予 public app tables/sequences；`app_schema_migrations`及其他 restore authority均不授予。

Default-branch authorization workflow 的 `production-first-deploy-e2e` 是獨立 fresh GitHub-hosted runner hard gate：它使用真正 `purpose=production` authorization、正式 bootstrap、host core、base Compose、80/443 及完整 HTTPS/WSS/admin/two-player smoke，並由正式 migration runner產生三筆固定 ID／SHA ledger 後才執行 first-installation claim，最後核對 signed first-deploy state 為 `consumed` 且 DB installation generation 為 2。每次 E2E 的 GitHub Deployment environment 名稱綁定 run id/attempt，避免不同 run 混雜。Deployment ID 建立後會在任何 authorization 生成前立即寫入、attest 及上傳唯一 source-binding artifact；reconciler 在 binding 存在時只直接取得該唯一 ID。如 runner 在 artifact 產生前硬取消，reconciler 只以 exact run environment 列出候選，將全部標為 error 並記錄 conflict；這條 fallback 絕不可發布 success。Job只收集 allowlisted authority摘要，不收完整 container log；Evidence與 cleanup完成後才由唯一 finalizer發布 terminal status。
