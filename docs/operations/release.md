...
3. 唯一受支援的正式發佈入口是 `./scripts/deploy-production.sh <artifact絕對目錄> <env絕對路徑>`。它會先驗證 artifact checksum、manifest 與 env 的三個 digest 完全相同，再依次執行 Compose config、pull 和 up。直接執行 `docker compose` 不受支援，也不得用來繞過 wrapper、migration 容器或映像核對。
...
- **向後相容 migration**：若上一版程式已在還原演練中證明可讀寫現有 schema，執行 `./scripts/prepare-application-rollback.sh <上版artifact> <現版artifact> <現行env> <新rollback artifact> <新rollback env>`。它只把 `SERVER_IMAGE` 及 `WEB_IMAGE` 回退，`DATABASE_IMAGE` 必須從現行已驗證 manifest 保留。再以 `deploy-production.sh` 及新 rollback artifact/env 執行，並重跑 smoke test。不可把舊 database image 直接指向現有 volume。
