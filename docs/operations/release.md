...
正式安裝 bootstrap 時 systemd 是強制 invariant：production 預設等同 `--install-systemd`，安裝器會驗證、複製及啟用 timer，立即執行 oneshot，並以 `/opt/steam-top-bootstrap/verify-reaper-health.sh` 核對 timer active、service Result 及 ExecMainStatus；任一步失敗必須 disable timer 並回滾 unit。只有 purpose 為 `release-integration` 才可明確使用 `--no-systemd-for-integration`。Legacy receipt 亦不得直接呼叫 raw script，只可經 `/opt/steam-top-bootstrap/import-legacy-cutover-current.sh ACTIVATION_NONCE LEGACY_NONCE RECEIPT RECEIPT_SIG READY READY_SHA PREFLIGHT PREFLIGHT_SIG`；它由 sealed current generation 選取 exact runtime，對舊 receipt 八個歷史欄位作 exact-schema 核對，再從已簽署 ready/preflight 與 live DB 推導 app role、ledger rows/hash。新 confirm 會在轉為 `verified` 的同一 transaction 保存 receipt payload、digest、原簽署及 signer；FS 遺失時只可重建原來 bytes，不得重簽。
