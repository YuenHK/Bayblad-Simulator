...
Production Compose 只將 PostgreSQL control port 綁到主機 loopback `127.0.0.1:15432`，供 root-owned、TLS `verify-full` 的 PGSERVICE 執行 claim、reaper 及 cutover；防火牆不得把此 port 轉送至 LAN。Migration及 first claim只用 DB owner control identity；pristine claim完成後才建立 `steam_top_app`（no-superuser/no-createdb/no-createrole/noinherit），server的 canonical `DATABASE_URL` 固定當前 production database，並以 Compose `!override` 只掛載獨立 app URL secret，owner secret僅供 migration/control。App provisioning在同一 advisory-lock transaction清除該 role 在所有 database 的 role settings 及顯式 CONNECT grant，但絕不修改共用叢集其他 database 的 `PUBLIC CONNECT`；在其他 database 亦不授予 app schema/object privileges。當前 database 內會撤銷角色繼承、schema/table/sequence/function/default privileges及 PUBLIC schema CREATE，再精確授予 public app tables/sequences；`app_schema_migrations`及其他 restore authority均不授予。
