...
外部簽署驗證完成後，pinned installer仍會在解壓前自行檢查每個 tar header：只接受 manifest唯一列出的單層 canonical relative regular files、root uid/gid、精確 mode、受限 size/count及逐檔 digest；absolute／`..`、symlink、hardlink、device及 FIFO一律在 `/opt`、`/etc`、`/var`首次 mutation前拒絕。執行檔名稱不屬信任資料，只有 running bytes對 signed manifest的 digest相等才可繼續。
