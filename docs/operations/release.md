...
Production Compose 只將 PostgreSQL control port 綁到主機 loopback `127.0.0.1:15432`，供 root-owned、TLS `verify-full` 的 PGSERVICE 執行 claim、reaper 及 cutover；防火牆不得把此 port 轉送至 LAN。Default-branch authorization workflow 的 `production-first-deploy-e2e` 是獨立 fresh GitHub-hosted runner hard gate：它使用真正 `purpose=production` authorization、正式 bootstrap、host core、base Compose、80/443 及完整 HTTPS/WSS/admin/two-player smoke，並由正式 migration runner產生三筆固定 ID／SHA ledger 後才執行 first-installation claim，最後核對 signed first-deploy state 為 `consumed` 且 DB installation generation 為 2。Job 無論結果都先收集不含 secret 的 container、migration、installation journal及 systemd evidence，再按 sealed runtime/project清理；只有整條鏈及 cleanup成功才由唯一 finalizer發布 success，其他終止狀態一律發布 failure。
