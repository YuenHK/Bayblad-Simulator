...
Production Compose 只將 PostgreSQL control port 綁到主機 loopback `127.0.0.1:15432`，供 root-owned、TLS `verify-full` 的 PGSERVICE 執行 claim、reaper 及 cutover；防火牆不得把此 port 轉送至 LAN。Default-branch authorization workflow 的 `production-first-deploy-e2e` 是獨立 fresh GitHub-hosted runner hard gate：它使用真正 `purpose=production` authorization、正式 bootstrap、host core、base Compose、80/443 及完整 HTTPS/WSS/admin/two-player smoke，最後核對 signed first-deploy state 為 `consumed` 且 DB installation generation 為 2。此 job 或其 cleanup/status 發布失敗會令整個 release authorization workflow 失敗。
