...
外部簽署驗證完成後，pinned installer 會先呼叫外置 pinned validator 檢查每個 tar header，再處理 validator 產生的受限 raw tar：只接受 manifest唯一列出的單層 canonical relative regular files、root uid/gid、精確 mode、受限 size/count及逐檔 digest；absolute／`..`、symlink、hardlink、device、FIFO、PAX、GNU long-name及 sparse metadata一律在 `/opt`、`/etc`、`/var`首次 mutation前拒絕。簽署壓縮檔上限為 8 MiB，串流解析另限制64個header、單檔2 MiB、總展開16 MiB及64倍壓縮比。執行檔名稱不屬信任資料，只有 running bytes對 signed manifest的 digest相等才可繼續。
...
Production Compose 只將 PostgreSQL control port 綁到主機 loopback `127.0.0.1:15432`，供 root-owned、TLS `verify-full` 的 PGSERVICE 執行 claim、reaper 及 cutover；防火牆不得把此 port 轉送至 LAN。Migration及 first claim只用 DB owner control identity；pristine claim完成後才建立 `steam_top_app`（no-superuser/no-createdb/no-createrole/noinherit），server以 Compose `!override` 只掛載獨立 app URL secret，owner secret僅供 migration/control。App provisioning在同一 advisory-lock transaction撤銷角色繼承、DB/schema/table/sequence/function/default privileges及PUBLIC schema CREATE，再精確授予 public app tables/sequences；`app_schema_migrations`及其他 restore authority均不授予，deployment probe另由受控cutover路徑授權。

Default-branch authorization workflow 的 `production-first-deploy-e2e` 是獨立 fresh GitHub-hosted runner hard gate：它使用真正 `purpose=production` authorization、正式 bootstrap、host core、base Compose、80/443 及完整 HTTPS/WSS/admin/two-player smoke，並由正式 migration runner產生三筆固定 ID／SHA ledger 後才執行 first-installation claim，最後核對 signed first-deploy state 為 `consumed` 且 DB installation generation 為 2。Deployment ID 建立後會在任何 authorization 生成前立即寫入、attest 及上傳唯一 source-binding artifact，綁定 run/attempt/workflow ref/SHA/environment/purpose/manifest/nonce；reconciler 只用這份 binding 取得唯一 Deployment ID，再與 terminal receipt 交叉驗證，不會枚舉 payload 作 authority。Job只收集 allowlisted container state／exit code、migration/installation authority摘要及 generation hash，不收完整 container log；上傳前以實際 secrets及敏感格式掃描，命中時只保留檔案 hash，保留期14日。Evidence與 cleanup完成後才由唯一 finalizer發布 terminal status；另有獨立 `workflow_run` reconciler處理 runner loss留下的無 terminal Deployment並標 error，永不把 orphan標 success。
