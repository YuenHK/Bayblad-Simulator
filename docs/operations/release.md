...
舊版 `committed` cutover migration 後只會成為 `legacy-committed`，絕不等同 APPROVED。管理員須保留歷史 one-stage schema v2 receipt、ready/sha、preflight 及兩份原簽署；receipt 必須是 purpose `production`、namespace `steam-top-cutover` 的14個 exact keys，`publicSmoke.passed` 必須是 boolean true。Canonical importer 會同時核對 live database、production marker 及 deletion ledger hash，並在同一 transaction 永久保存 receipt/ready/preflight 的 payload、digest、signature 及 signer；欠任一證據必須保持 legacy 狀態。

正式安裝 bootstrap 時 systemd 是強制 invariant：production 預設等同 `--install-systemd`，安裝器會驗證、複製及啟用 timer，立即執行 oneshot，並以 `/opt/steam-top-bootstrap/verify-reaper-health.sh` 核對 timer active、service Result 及 ExecMainStatus；全新主機尚無 current/protected state/pending outbox 時只可回報 `not-applicable-clean-host`，首次部署建立 current 後 record workflow 必須使用 `--require-active-runtime`。任一步失敗必須 disable timer 並回滾 unit。只有 purpose 為 `release-integration` 才可明確使用 `--no-systemd-for-integration`。Legacy receipt 亦不得直接呼叫 raw script，只可經 `/opt/steam-top-bootstrap/import-legacy-cutover-current.sh ACTIVATION_NONCE LEGACY_NONCE RECEIPT RECEIPT_SIG READY READY_SHA PREFLIGHT PREFLIGHT_SIG`；confirm 輸出由 wrapper 固定在 root-owned 0700 incident state directory，不接受 caller 任意路徑。
