...
2. `NODE_IMAGE`、`POSTGRES_IMAGE`、`CADDY_IMAGE` 必須填入由 CI 核實的 `registry/name@sha256:<64 hex>`，不可使用 tag 或自行猜測 digest。
...
4. 校方指定教師帳號為 `admin`，初始密碼為 `REDACTED_EXPOSED_PASSWORD`；只填入正式 `.env`，不要提交。
