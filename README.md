...
在設計室「即時計算」下方按 **「傳送至 ShapeCut」**，新分頁會自動接收三層板材，無須下載再上載。接收後仍由你選擇 **6 mm 板材**、按「開始製作」，核對分層預覽後輸出 DXF。手機先切換至「預測結果」頁籤。

傳送只在兩個瀏覽器分頁之間進行，不上傳模型到伺服器；接收分頁不會覆寫 ShapeCut 原有儲存專案。若新分頁被瀏覽器阻擋、已關閉或傳送逾時，可重試，或按 **「下載 STL（供 ShapeCut）」** 後在 [ShapeCut](https://yuenhk.github.io/ShapeCut/) 手動選擇檔案。「停止等待」只停止本頁等候；已送出的板材仍可能在 ShapeCut 匯入，請到該分頁確認。
